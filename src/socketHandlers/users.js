...
const rankPerks = require('../rankPerks');
...
module.exports = function register(socket, ctx) {
  ...
  // ── DM Nudge / Buzz (#6) ────────────────────────────────
  // Buzz the other person in a DM: their screen shakes + vibrates + notifies.
  // Only works inside a real DM the sender belongs to, so it can't be used to
  // ping arbitrary users. Rate-limited to one buzz every 4s per connection.
  let _lastNudge = 0;
  socket.on('dm:nudge', (data) => {
    if (!data || typeof data.code !== 'string') return;
    if (socket.user.isGuest) return socket.emit('error-msg', 'Guests cannot nudge');
    const now = Date.now();
    if (now - _lastNudge < 4000) return;
    _lastNudge = now;
    const ch = db.prepare('SELECT id FROM channels WHERE code = ? AND is_dm = 1').get(data.code);
    if (!ch) return;
    const mine = db.prepare('SELECT 1 FROM channel_members WHERE channel_id = ? AND user_id = ?').get(ch.id, socket.user.id);
    if (!mine) return;
    const other = db.prepare('SELECT user_id FROM channel_members WHERE channel_id = ? AND user_id != ?').get(ch.id, socket.user.id);
    if (!other) return;   // self-DM: no one to buzz
    const me = db.prepare('SELECT COALESCE(display_name, username) AS n FROM users WHERE id = ?').get(socket.user.id);
    const fromName = (me && me.n) || socket.user.username;
    io.to('user:' + other.user_id).emit('dm:nudged', { code: data.code, fromId: socket.user.id, from: fromName });
    socket.emit('dm:nudge-sent', { code: data.code });
  });
    ...
    try {
      const row = db.prepare(
        `SELECT u.id, u.username, COALESCE(u.display_name, u.username) as displayName,
                ...
                u.profile_banner, u.name_effect
      ...
      socket.emit('user-profile', {
        ...
        // Rank-reward cosmetics + the level that gates them (so the Edit Profile
        // picker can show which banners/effects are still locked).
        profileBanner: row.profile_banner || null,
        nameEffect: row.name_effect || null,
        level: (ctx.xp && ctx.xp.getUserXp) ? (ctx.xp.getUserXp(data.userId).level || 0) : 0,
  ...
  // ── Gamer discovery profile: country (region) + languages ──────
  // Short codes only (no free text → clean, filterable facets for player search).
  socket.on('set-country', (data) => {
    if (!data || typeof data.country !== 'string') return;
    const raw = data.country.trim().toUpperCase();
    const country = /^[A-Z]{2,3}$/.test(raw) ? raw : '';
    try {
      db.prepare('UPDATE users SET country = ? WHERE id = ?').run(country || null, socket.user.id);
      socket.emit('profile-updated', { country });
    } catch (err) { console.error('Set country error:', err); }
  });

  socket.on('set-languages', (data) => {
    if (!data) return;
    const list = Array.isArray(data.languages) ? data.languages : String(data.languages || '').split(',');
    const langs = [...new Set(list.map(x => String(x).trim().toLowerCase()).filter(x => /^[a-z]{2,3}$/.test(x)))].slice(0, 8);
    const csv = langs.join(',');
    try {
      db.prepare('UPDATE users SET languages = ? WHERE id = ?').run(csv || null, socket.user.id);
      socket.emit('profile-updated', { languages: csv });
    } catch (err) { console.error('Set languages error:', err); }
  });

  // ── Rank-reward cosmetics (server-authoritative; earned by level) ──────
  const _rankLevel = () => (ctx.xp && ctx.xp.getUserXp ? (ctx.xp.getUserXp(socket.user.id).level || 0) : 0);
  socket.on('set-profile-banner', (data) => {
    if (!data || typeof data.banner !== 'string') return;
    const id = data.banner;
    try {
      if (!rankPerks.bannerAllowed(_rankLevel(), id)) { socket.emit('error-msg', 'That banner unlocks at a higher rank.'); return; }
      const store = id === 'default' ? null : id;
      db.prepare('UPDATE users SET profile_banner = ? WHERE id = ?').run(store, socket.user.id);
      socket.emit('profile-updated', { profileBanner: store });
    } catch (err) { console.error('Set banner error:', err); }
  });
  socket.on('set-name-effect', (data) => {
    if (!data || typeof data.effect !== 'string') return;
    const id = data.effect;
    try {
      if (!rankPerks.nameEffectAllowed(_rankLevel(), id)) { socket.emit('error-msg', 'That name effect unlocks at a higher rank.'); return; }
      const store = id === 'none' ? null : id;
      db.prepare('UPDATE users SET name_effect = ? WHERE id = ?').run(store, socket.user.id);
      socket.emit('profile-updated', { nameEffect: store });
    } catch (err) { console.error('Set name effect error:', err); }
  });
