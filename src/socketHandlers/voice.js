...
module.exports = function register(socket, ctx) {
  const { io, db, state, userHasPermission, getUserEffectiveLevel, getUserHighestRole,
          ...
          getActiveMusicSyncState, getMusicQueuePayload, botAudioManager,
          sfu, voiceProviders, getVoiceSetting } = ctx;
  ...
  socket.on('voice-join', (data) => {
    ...
    // ── Media provider: mesh (default) or SFU. Fixed for the room's lifetime.
    // The presence events fire either way; only the client's MEDIA path forks
    // on this. (v1: SFU is all-or-nothing per server via the admin toggle;
    // per-room auto-upgrade mid-call is future work.)
    if (sfu && voiceProviders) {
      let provider = voiceProviders.get(code);
      if (!provider) {
        provider = (getVoiceSetting && getVoiceSetting('sfu_enabled') === 'true') ? 'sfu' : 'p2p';
        voiceProviders.set(code, provider);
      }
      socket.emit('voice-provider-config', { channelCode: code, provider });
      if (provider === 'sfu') {
        Promise.resolve(sfu.join(code, socket.user.id)).catch(err => console.warn('[sfu] join failed:', err && err.message));
      }
    }
  ...
  // ── SFU signaling (only used when a room is on the SFU provider) ─────────
  // The SFU is the sole offerer; the client only ever answers and trickles ICE.
  const inSfuRoom = (code) => voiceProviders && voiceProviders.get(code) === 'sfu' && voiceUsers.get(code)?.has(socket.user.id);

  socket.on('sfu-answer', (data) => {
    if (!sfu || !data || !isString(data.code, 8, 8) || !data.sdp) return;
    if (!inSfuRoom(data.code)) return;
    Promise.resolve(sfu.handleAnswer(data.code, socket.user.id, data.sdp)).catch(() => {});
  });

  socket.on('sfu-ice', (data) => {
    if (!sfu || !data || !isString(data.code, 8, 8) || !data.candidate) return;
    if (typeof data.candidate !== 'object' || JSON.stringify(data.candidate).length > MAX_ICE_SIZE) return;
    if (!inSfuRoom(data.code)) return;
    Promise.resolve(sfu.handleIce(data.code, socket.user.id, data.candidate)).catch(() => {});
  });

  // Client wants a fresh publish slot (screen share / webcam / music) — the SFU
  // adds a receive transceiver and re-offers.
  socket.on('sfu-publish', (data) => {
    if (!sfu || !data || !isString(data.code, 8, 8)) return;
    if (!inSfuRoom(data.code)) return;
    Promise.resolve(sfu.requestPublish(data.code, socket.user.id, data.kind === 'video' ? 'video' : 'audio')).catch(() => {});
  });
  ...
  socket.on('voice-kick', (data) => {
    ...
    socket.emit('toast', { message: `Kicked ${target.username} from voice`, type: 'success' });
  });
