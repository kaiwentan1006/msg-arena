...
module.exports = function register(socket, ctx) {
  const {
    ...
    transferAdminRef, HAVEN_VERSION, logAudit, getAdminRoleDisplay, xp
  } = ctx;
  ...
  // ── Create role ─────────────────────────────────────────
  // ── Level role rewards (config) ─────────────────────────
  // Reach a configured level, auto-earn a role — the headline engagement lever
  // of every gamer leveling system. Admin/manage_roles only. Granting on
  // level-up happens in xpService; here we configure the mapping and (MEE6-style)
  // apply it to members who ALREADY qualify.
  const _canManageLevelRoles = () => socket.user.isAdmin || userHasPermission(socket.user.id, 'manage_roles');

  socket.on('xp:list-level-roles', (data, callback) => {
    const cb = typeof callback === 'function' ? callback : (typeof data === 'function' ? data : () => {});
    if (!_canManageLevelRoles()) return cb({ error: 'Not allowed' });
    cb({ levelRoles: xp ? xp.listLevelRoles() : [] });
  });

  socket.on('xp:set-level-role', (data, callback) => {
    const cb = typeof callback === 'function' ? callback : () => {};
    if (!_canManageLevelRoles()) return cb({ error: 'Not allowed' });
    if (!xp || !data) return cb({ error: 'Unavailable' });
    const level = parseInt(data.level, 10);
    const roleId = parseInt(data.roleId, 10);
    if (!xp.setLevelRole(level, roleId)) return cb({ error: 'Invalid level or role' });
    // Give the role to members who already reached this level.
    let granted = 0;
    try {
      const g = db.prepare('INSERT OR IGNORE INTO user_roles (user_id, role_id, channel_id) VALUES (?, ?, NULL)');
      for (const row of db.prepare('SELECT user_id FROM user_xp WHERE level >= ?').all(level)) {
        if (g.run(row.user_id, roleId).changes > 0) granted++;
      }
    } catch { /* backfill best-effort */ }
    try { for (const [code] of channelUsers) emitOnlineUsers(code); } catch {}
    _audit(socket.user.id, 'xp_level_role_set', `level ${level} -> role ${roleId} (${granted} backfilled)`);
    cb({ ok: true, granted, levelRoles: xp.listLevelRoles() });
  });

  socket.on('xp:remove-level-role', (data, callback) => {
    const cb = typeof callback === 'function' ? callback : () => {};
    if (!_canManageLevelRoles()) return cb({ error: 'Not allowed' });
    if (!xp || !data) return cb({ error: 'Unavailable' });
    xp.removeLevelRole(parseInt(data.level, 10)); // keeps roles members already earned
    _audit(socket.user.id, 'xp_level_role_removed', `level ${data.level}`);
    cb({ ok: true, levelRoles: xp.listLevelRoles() });
  });
