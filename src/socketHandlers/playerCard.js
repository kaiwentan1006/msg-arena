  ...
  socket.on('get-player-card', (data) => {
    ...
    try {
      ...
      socket.emit('player-card', {
        ...
        games: db.prepare(`
          SELECT g.slug, g.name, g.icon, ug.rank FROM user_games ug JOIN games g ON g.id = ug.game_id
          WHERE ug.user_id = ? AND g.is_active = 1 ORDER BY g.name LIMIT 12
        `).all(uid),
        profile: (() => { const u = db.prepare('SELECT country, languages FROM users WHERE id = ?').get(uid) || {};
          return { country: u.country || null, languages: u.languages ? String(u.languages).split(',').filter(Boolean) : [] }; })(),
        squads: db.prepare(`
