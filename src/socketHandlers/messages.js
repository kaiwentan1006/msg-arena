...
module.exports = function register(socket, ctx) {
  const { io, db, state, userHasPermission, getUserEffectiveLevel, getChannelRoleChain,
          ...
          UPLOADS_DIR, DELETED_ATTACHMENTS_DIR, xp, isXpEnabled, emitOnlineUsers } = ctx;
  const { slowModeTracker } = state;

  // Award activity XP to the message sender (server-authoritative; per-user
  // cooldown lives in the service, so a message then a poll won't double-award).
  function _awardMessageXp() {
    try {
      if (!xp || (isXpEnabled && !isXpEnabled())) return;
      const r = xp.awardMessage(socket.user && socket.user.id);
      if (!r) return;
      socket.emit('xp-update', { xp: r.xp, level: r.level, streak: r.streak });
      if (r.leveledUp) socket.emit('xp-levelup', { level: r.level, xp: r.xp, rewardedRoles: r.rewardedRoles || [] });
      // First activity of a new day → celebrate the streak.
      if (r.newDay && r.streak > 1) socket.emit('xp-streak', { streak: r.streak, bonus: r.streakBonus });
      // A level-reward role was just granted → refresh the member roster so the
      // new badge/colour shows for everyone in the channel (permission is already
      // live in the DB regardless of this refresh).
      if (r.rewardedRoles && r.rewardedRoles.length && socket.currentChannel && emitOnlineUsers) {
        emitOnlineUsers(socket.currentChannel);
      }
    } catch { /* best-effort; never break message send */ }
  }
  ...
  socket.on('send-message', (data) => {
    ...
    // prefix for this step. The target is stripped from what MSG Arena stores and
    ...
    try {
      ...
      // Deliberately after the broadcast: the MSG Arena message is already sent and
  ...
  socket.on('mark-read', (data) => {
    ...
    try {
      ...
      // DM read receipts (#5): tell the other member their messages up to here are
      // now read, so their sent-ticks can flip to read-ticks (Telegram/WhatsApp).
      if (channel.is_dm) {
        const others = db.prepare('SELECT user_id FROM channel_members WHERE channel_id = ? AND user_id != ?').all(channel.id, socket.user.id);
        for (const o of others) {
          io.to('user:' + o.user_id).emit('dm:read', { code, readerId: socket.user.id, lastReadMessageId: data.messageId });
        }
      }
    } catch (err) {
  ...
  // DM read-state request (#5): when you open a DM, learn how far the OTHER
  // person has read YOUR messages, so already-read ticks show immediately.
  socket.on('dm:read-state', (data) => {
    if (!data || typeof data.code !== 'string') return;
    const code = data.code.trim();
    if (!/^[a-f0-9]{8}$/i.test(code)) return;
    const channel = db.prepare('SELECT id, is_dm FROM channels WHERE code = ?').get(code);
    if (!channel || !channel.is_dm) return;
    const mine = db.prepare('SELECT 1 FROM channel_members WHERE channel_id = ? AND user_id = ?').get(channel.id, socket.user.id);
    if (!mine) return;
    const other = db.prepare('SELECT user_id FROM channel_members WHERE channel_id = ? AND user_id != ?').get(channel.id, socket.user.id);
    if (!other) return;
    const rp = db.prepare('SELECT last_read_message_id FROM read_positions WHERE user_id = ? AND channel_id = ?').get(other.user_id, channel.id);
    socket.emit('dm:read', { code, readerId: other.user_id, lastReadMessageId: rp ? rp.last_read_message_id : 0 });
  });
