...
module.exports = (function () {
  // perf (MEDIUM 3): the permission_thresholds blob is read + JSON.parsed inside
  // every userHasPermission call, which runs thousands of times per broadcast.
  // Cache it briefly (module scope so it survives the per-request factory).
  let _thrCache = null, _thrExpires = 0;
  return function createPermissions(db) {
  ...
  function getPermissionThresholds() {
    const now = Date.now();
    if (_thrCache && now < _thrExpires) return _thrCache;
    try {
      const row = db.prepare("SELECT value FROM server_settings WHERE key = 'permission_thresholds'").get();
      _thrCache = row ? JSON.parse(row.value) : {};
    } catch { _thrCache = {}; }
    _thrExpires = now + 5000;  // 5s TTL — thresholds change rarely (admin action)
    return _thrCache;
  }
