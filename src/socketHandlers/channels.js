...
module.exports = function register(socket, ctx) {
  ...
  socket.on('delete-sub-channel', (data) => {
    ...
    try {
      ...
      broadcastChannelLists();
      socket.emit('toast', { message: 'Sub-channel deleted', type: 'success' });
    } catch (err) {
  ...
  socket.on('set-channel-category', (data) => {
    ...
    try {
      ...
      broadcastChannelLists();
      socket.emit('toast', { message: category ? `Category set to "${category}"` : 'Category removed', type: 'success' });
    } catch (err) {
  ...
  socket.on('update-channel-code-settings', (data) => {
    ...
    socket.emit('toast', { message: 'Channel code settings updated', type: 'success' });
  });
  ...
  socket.on('remove-from-channel', (data, callback) => {
    ...
    cb({ success: true });
    socket.emit('toast', { message: `Removed ${targetUser.username} from #${channel.name}`, type: 'success' });
  });
