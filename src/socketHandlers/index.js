...
const registerAdminUsers  = require('./adminUsers');
...
function setupSocketHandlers(io, db, opts = {}) {
      ...
      for (const [code, room] of voiceUsers) {
        ...
        for (const [uid, u] of room) {
          ...
          if (r && u.socketId) {
            ...
            if (r.leveledUp) io.to(u.socketId).emit('xp-levelup', { level: r.level, xp: r.xp, rewardedRoles: r.rewardedRoles || [] });
            if (r.rewardedRoles && r.rewardedRoles.length) rolesGranted = true;
          }
        }
        // A voice level-up handed out a reward role → refresh that channel's
        // roster so the new badge shows (permission is already live in the DB).
        if (rolesGranted) { try { emitOnlineUsers(code); } catch {} }
      }
  ...
  function getEnrichedChannels(userId, isAdmin, joinRooms) {
    // Online set for DM partners, built once per call (the sidebar shows each
    // DM partner's avatar + presence dot). Lazily populated on the first DM row.
    let _dmOnline = null;
    ...
    if (channels.length > 0) {
      ...
      channels.forEach(ch => {
        ...
        if (ch.is_dm) {
          if (!_dmOnline) {
            _dmOnline = new Set();
            for (const [, s] of io.of('/').sockets) if (s.user) _dmOnline.add(s.user.id);
          }
          const otherUser = db.prepare(`
            SELECT u.id, COALESCE(u.display_name, u.username) as username, u.avatar, u.avatar_shape FROM users u
            JOIN channel_members cm ON u.id = cm.user_id
            WHERE cm.channel_id = ? AND u.id != ?
          `).get(ch.id, userId);
          if (otherUser) {
            otherUser.online = _dmOnline.has(otherUser.id);
            ch.dm_target = otherUser;
          } else {
            // Self-DM: only one channel_members row, no "other" user. Use self as the partner.
            const self = db.prepare(
              'SELECT id, COALESCE(display_name, username) as username, avatar, avatar_shape FROM users WHERE id = ?'
            ).get(userId);
  ...
  io.use((socket, next) => {
    ...
    // Record which client this member is on (web / desktop app / Android app) so
    // the admin user page can show it. Cheap best-effort; never blocks auth.
    try {
      const _ua = (socket.handshake && socket.handshake.headers && socket.handshake.headers['user-agent']) || '';
      const _client = /MSGArenaAndroid/i.test(_ua) ? 'Android app'
        : /Haven|Electron/i.test(_ua) ? 'Desktop app' : 'Web browser';
      db.prepare('UPDATE users SET last_client = ? WHERE id = ?').run(_client, user.id);
  ...
  io.on('connection', (socket) => {
    ...
    // ── Referral ranks ──────────────────────────────────────
    // Recruiting friends raises your rank: reward the inviter with XP for every
    // NEW member who joined through their invite links. Done lazily on connect,
    // so an inviter who was offline when someone joined still gets it next time.
    try {
      const _uid = socket.user.id;
      const _refs = db.prepare(
        'SELECT COUNT(*) c FROM invite_code_uses u JOIN invite_codes ic ON ic.id = u.invite_code_id WHERE ic.created_by = ? AND u.user_id != ?'
      ).get(_uid, _uid).c;
      const _rewarded = (db.prepare('SELECT referral_rewarded FROM users WHERE id = ?').get(_uid) || {}).referral_rewarded || 0;
      if (_refs > _rewarded) {
        const _gained = _refs - _rewarded;
        db.prepare('UPDATE users SET referral_rewarded = ? WHERE id = ?').run(_refs, _uid);
        if (_xpEnabled && xpService) {
          const r = xpService.awardReferral(_uid, _gained);
          if (r) {
            socket.emit('xp-referral', { newReferrals: _gained, totalReferrals: _refs, xp: r.xp, level: r.level });
            socket.emit('xp-update', { xp: r.xp, level: r.level });
            if (r.leveledUp) socket.emit('xp-levelup', { level: r.level, xp: r.xp, rewardedRoles: r.rewardedRoles || [] });
            if (r.rewardedRoles && r.rewardedRoles.length && socket.currentChannel) { try { emitOnlineUsers(socket.currentChannel); } catch {} }
          }
        }
      }
    } catch (e) { /* referral reward is best-effort */ }
