...
module.exports = function register(socket, ctx) {
  ...
  // Create (or reuse) the party's dedicated voice channel WITHOUT notifying —
  // the members-only room that gives "joined" a real meaning vs "not joined" (#16).
  function getOrCreatePartyVoice(post) {
    if (post.voice_code) {
  ...
  // Spin up (or reuse) the party's voice channel and pull ALL members in (used
  // when the party fills or someone hits "start voice").
  function ensurePartyVoice(post) {
    const existed = !!(post.voice_code &&
      db.prepare('SELECT code FROM channels WHERE code = ?').get(post.voice_code));
    const code = getOrCreatePartyVoice(post);
    const game = db.prepare('SELECT name FROM games WHERE id = ?').get(post.game_id) || { name: 'Party' };
    ...
    if (!existed) {
      // Only nudge (push) the first time the room appears, so re-calls don't spam.
      sendUserPush(
  ...
  socket.on('lfg:join', (data) => {
    ...
    if (nowFilled >= post.slots) {
      ...
      // As soon as a second person joins, open the dedicated party voice room and
      // hand the code to everyone in the party — so joining actually puts you in a
      // shared space to coordinate, instead of doing nothing until the party is
      // full (#16). Non-members never receive the code.
      const fresh = db.prepare('SELECT * FROM lfg_posts WHERE id = ?').get(post.id);
      const code = getOrCreatePartyVoice(fresh);
      const memberIds = db.prepare('SELECT user_id FROM lfg_slots WHERE post_id = ?').all(post.id).map(r => r.user_id);
      emitToUsers(io, memberIds, 'lfg:party-ready', { postId: post.id, voiceCode: code });
    }
