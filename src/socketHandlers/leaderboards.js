...
module.exports = function register(socket, ctx) {
  ...
  socket.on('get-leaderboards', () => {
    try {
      ...
      // ── Top recruiters — most friends brought in via invite links ──
      const recruiters = db.prepare(`
        SELECT ic.created_by AS userId, COUNT(*) AS count
        FROM invite_code_uses u JOIN invite_codes ic ON ic.id = u.invite_code_id
        WHERE ic.created_by IS NOT NULL AND u.user_id != ic.created_by
        GROUP BY ic.created_by ORDER BY count DESC LIMIT ?
      `).all(TOP).map((r, i) => ({ rank: i + 1, userId: r.userId, name: nameOf(r.userId), count: r.count }));

      socket.emit('leaderboards', { titles, clips, ladders, arcade, levels, recruiters });
    } catch (e) {
