...
// Is this a private / reserved / non-routable address? Such IPs (loopback,
// LAN ranges, CGNAT 100.64/10 that PaaS proxies like Railway use, link-local)
// can't be geolocated — sending them to a geo API returns a bogus datacenter
// country, which is exactly the "completely wrong country" admins saw (#20).
function isPrivateIp(ip) {
  const a = normalizeIp(ip);
  if (!a) return true;
  if (isIPv4(a)) {
    const n = ipToBigInt(a);
    if (n === null) return true;
    const inRange = (net, bits) => (n >> BigInt(32 - bits)) === (ipToBigInt(net) >> BigInt(32 - bits));
    return (
      inRange('0.0.0.0', 8)      ||  // "this" network
      inRange('10.0.0.0', 8)     ||  // private
      inRange('100.64.0.0', 10)  ||  // CGNAT (Railway/Fly and other PaaS edges)
  ...
  if (isIPv6(a)) {
    ...
    if (/^f[cd]/i.test(a)) return true;       // fc00::/7 unique-local
    return false;
  }
