...
function initDatabase() {
  ...
  // ── Migration: gamer discovery profile fields (country + languages) ──
  // Powers player search ("find teammates by game + region + language"). country
  // is a short region code (e.g. 'DE'); languages is a CSV of codes ('en,de').
  try {
    db.prepare("SELECT country FROM users LIMIT 0").get();
  } catch {
    db.exec("ALTER TABLE users ADD COLUMN country TEXT DEFAULT NULL");
    db.exec("ALTER TABLE users ADD COLUMN languages TEXT DEFAULT NULL");
  }

  // ── Migration: referral reward tracking ──────────────────
  // Recruiting friends raises your rank. We reward the inviter with XP for each
  // NEW person who joined via their invite links; referral_rewarded is how many
  // of those we've already granted XP for (so we never double-reward).
  try {
    db.prepare("SELECT referral_rewarded FROM users LIMIT 0").get();
  } catch {
    db.exec("ALTER TABLE users ADD COLUMN referral_rewarded INTEGER NOT NULL DEFAULT 0");
  }

  // ── Migration: rank-reward cosmetics (profile banner + name effect) ──
  // Preset ids from src/rankPerks.js; NULL = the default. Unlocked by level.
  try { db.prepare("SELECT profile_banner FROM users LIMIT 0").get(); }
  catch { db.exec("ALTER TABLE users ADD COLUMN profile_banner TEXT DEFAULT NULL"); }
  try { db.prepare("SELECT name_effect FROM users LIMIT 0").get(); }
  catch { db.exec("ALTER TABLE users ADD COLUMN name_effect TEXT DEFAULT NULL"); }

  // ── Migration: last client type (web / desktop app / Android app) ──
  // Recorded on connect so the admin user page can show what each member uses.
  try { db.prepare("SELECT last_client FROM users LIMIT 0").get(); }
  catch { db.exec("ALTER TABLE users ADD COLUMN last_client TEXT DEFAULT NULL"); }
  ...
  db.exec(`
    ...
    -- Level role rewards: reach a level, auto-earn a role (the headline
    -- engagement feature of every gamer leveling system). One role per level;
    -- the role FK cascades so deleting a role clears its reward mapping.
  ...
  // ── Migration: daily-activity streak columns on user_xp ──
  // Gamers return daily to keep a streak alive — the biggest retention lever in
  // the leveling research. Stored on the XP row so one record holds a member's
  // whole activity profile.
  try {
    db.prepare("SELECT last_active_day FROM user_xp LIMIT 0").get();
  } catch (e) {
    db.exec("ALTER TABLE user_xp ADD COLUMN last_active_day TEXT DEFAULT NULL");
    db.exec("ALTER TABLE user_xp ADD COLUMN streak INTEGER NOT NULL DEFAULT 0");
    db.exec("ALTER TABLE user_xp ADD COLUMN longest_streak INTEGER NOT NULL DEFAULT 0");
  }
  ...
  `);
  // Migration: per-game rank/skill for existing user_games rows (discovery search).
  try {
    db.prepare("SELECT rank FROM user_games LIMIT 0").get();
  } catch {
    db.exec("ALTER TABLE user_games ADD COLUMN rank TEXT DEFAULT NULL");
  }
