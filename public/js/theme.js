// ═══════════════════════════════════════════════════════════
// MSG Arena — Shared Theme Switcher (loaded on all pages)
// ═══════════════════════════════════════════════════════════
...
let _scrambleTimer = null, _scrambleOriginal = 'MSG ARENA';
...
function _collectScrambleTargets() {
  ...
  // 1. Brand text (MSG ARENA logo) — always included
...
// Draw a six-armed snow crystal centred at (x,y) with arm-length r, rotated by
// `rot`. Each arm has two pairs of side branches, like a real dendrite. Cheap
// enough to run per-flake at 30fps (six spokes of a few short strokes).
function _drawSnowCrystal(ctx, x, y, r, rot) {
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(rot || 0);
  ...
  for (let a = 0; a < 6; a++) {
    ...
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.lineTo(0, -r);                       // main spoke
    ctx.moveTo(0, -r * 0.60); ctx.lineTo(r * 0.30, -r * 0.86);   // outer branch
    ctx.moveTo(0, -r * 0.60); ctx.lineTo(-r * 0.30, -r * 0.86);
    ctx.moveTo(0, -r * 0.34); ctx.lineTo(r * 0.24, -r * 0.55);   // inner branch
    ctx.moveTo(0, -r * 0.34); ctx.lineTo(-r * 0.24, -r * 0.55);
    ctx.stroke();
  }
  ctx.restore();
}
...
function _startNordSnow() {
  ...
  function spawnFlake() {
    _snowflakes.push({
      ...
      rot: Math.random() * Math.PI * 2,             // crystal orientation
      spin: (Math.random() - 0.5) * 0.03,           // gentle tumble
  ...
  function draw(now) {
    ...
    for (let i = _snowflakes.length - 1; i >= 0; i--) {
      ...
      // Draw an actual six-armed snow crystal (not a plain dot). Tiny/distant
      // flakes stay as soft specks — cheap, and that's how far snow reads anyway.
      _snowCtx.fillStyle = '#eaf1fb';
      _snowCtx.strokeStyle = '#eaf1fb';
      if (f.size < 1.7) {
        ...
        // soft glow halo
        _snowCtx.globalAlpha = alpha * 0.16;
        _snowCtx.beginPath();
        _snowCtx.arc(f.x, f.y, f.size * 2.1, 0, Math.PI * 2);
        _snowCtx.fill();
        // the crystal
        _snowCtx.globalAlpha = alpha;
        _drawSnowCrystal(_snowCtx, f.x, f.y, f.size * 2.4, f.rot);
      }
    }
...
function initCustomThemeEditor() {
  ...
  // Draw the editor's own canvases on init, but do NOT repaint the whole
  // document (apply()) unless the custom theme is actually active. The
  // settings panel — and therefore this editor — is present in the DOM on
  // every app load, so an unconditional render() here would override the
  // current theme's accent with the editor's default hue (and persist it),
  // which is what made the app render violet while the login page stayed on
  // the real theme accent.
  redrawHue();
  redrawTri();
  if ((localStorage.getItem('haven_theme') || 'haven') === 'custom') apply();
