// ═══════════════════════════════════════════════════════════
// MSG Arena — WebRTC Voice Chat Manager
// ═══════════════════════════════════════════════════════════
...
class VoiceManager {
  constructor(socket) {
    ...
    // ── SFU (optional; set per room by the server via voice-provider-config) ──
    this.provider = 'p2p';          // 'p2p' (mesh, default) | 'sfu'
    this._sfuPc = null;             // the single RTCPeerConnection to the server SFU
    this._sfuTrackMap = {};         // transceiver mid → { userId, kind } (who each remote track is)
    this._sfuMicMids = new Set();   // publish-slot mids we've already attached our mic to
    ...
    // Result was every MSG Arena server using default ICE config lost external
  ...
  // "MSG Arena broke web calls" rather than "this STUN server is gone", and it cost
  // one admin days (#5542).
  //
  ...
  async _probeConfiguredStun(iceServers) {
    try {
      ...
      if (live.length) {
        ...
        console.warn(
          `[Voice] Falling back to MSG Arena's built-in STUN pool for this session (${live.join(', ')}). ` +
      ...
      if (!this._connectivityWarned && typeof this.onConnectivityWarning === 'function') {
        ...
        this.onConnectivityWarning(live.length
          ? `None of this server's configured STUN servers are responding (${dead.join(', ')}). ` +
            'MSG Arena is using its built-in servers for now so calls keep working, but an admin ' +
            'should fix or clear that setting in Settings, Voice & Connectivity.'
          : `Cannot reach any STUN server (${dead.join(', ')}) from this device. ` +
            'This is almost always a VPN or firewall blocking WebRTC on THIS device (for example a VPN ' +
            'WebRTC / IP-leak protection, as in NordVPN) rather than a server outage. Turn the VPN off or ' +
            'exclude MSG Arena from it, or add a TURN relay in Settings, Voice & Connectivity for calls that ' +
            'cross networks. Until then, voice only works with people on your local network.'
        );
      }
    } catch (err) {
  ...
  async _probeDefaultStun() {
    try {
      ...
      if (!this._connectivityWarned && typeof this.onConnectivityWarning === 'function') {
        this._connectivityWarned = true;
        this.onConnectivityWarning('Cannot reach the voice connection servers (STUN) from this device. This is usually a VPN or firewall blocking WebRTC (for example a VPN WebRTC / IP-leak protection) rather than a server problem: turn the VPN off or exclude MSG Arena from it. For reliable calls across networks, an admin can add a TURN relay in Settings → Voice & Connectivity. Local-network calls still work.');
      }
    } catch (err) {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // SFU adapter — used when the server puts a voice room on the SFU provider.
  // We keep ONE RTCPeerConnection to the server: it uploads our mic once and
  // forwards everyone else's audio down. The SFU is the sole offerer, so we
  // only ever answer and trickle ICE — no glare. Presence/mute/tiles keep
  // using the same events as mesh; only the media path changes here. v1 is
  // audio-first (voice scaling); forwarded video is best-effort.
  // ═══════════════════════════════════════════════════════════════════════
  _setupSfuListeners() {
    this.socket.on('voice-provider-config', (data) => {
      if (!data) return;
      this.provider = data.provider === 'sfu' ? 'sfu' : 'p2p';
    });
    this.socket.on('sfu-offer', (data) => {
      this._sfuHandleOffer(data).catch(e => console.warn('[sfu] offer failed:', e && e.message));
    });
    this.socket.on('sfu-ice', async (data) => {
      if (!this._sfuPc || !data || !data.candidate) return;
      try { await this._sfuPc.addIceCandidate(data.candidate); } catch { /* transport gone */ }
    });
  }

  _sfuEnsurePc() {
    if (this._sfuPc) return this._sfuPc;
    const pc = new RTCPeerConnection(this.rtcConfig);
    pc.onicecandidate = (e) => {
      if (e.candidate && this.currentChannel) {
        this.socket.emit('sfu-ice', {
          code: this.currentChannel,
          candidate: e.candidate.toJSON ? e.candidate.toJSON() : e.candidate,
        });
      }
    };
    pc.ontrack = (event) => this._sfuOnTrack(event);
    this._sfuPc = pc;
    return pc;
  }

  async _sfuHandleOffer(data) {
    if (!data || !data.sdp || this.provider !== 'sfu') return;
    const pc = this._sfuEnsurePc();
    this._sfuTrackMap = data.trackMap || {};
    await pc.setRemoteDescription(data.sdp);
    // Attach our mic to each publish slot the SFU opened for us.
    const micTrack = this.localStream && this.localStream.getAudioTracks()[0];
    for (const [mid, kind] of Object.entries(data.sendMids || {})) {
      if (this._sfuMicMids.has(mid)) continue;
      const tx = pc.getTransceivers().find(t => t.mid === mid);
      if (tx && kind === 'audio' && micTrack) {
        try { await tx.sender.replaceTrack(micTrack); this._sfuMicMids.add(mid); }
        catch (e) { console.warn('[sfu] mic attach failed:', e && e.message); }
      }
    }
    const answer = await pc.createAnswer();
    await pc.setLocalDescription(answer);
    this.socket.emit('sfu-answer', { code: this.currentChannel, sdp: pc.localDescription });
  }

  _sfuOnTrack(event) {
    const track = event.track;
    const mid = event.transceiver && event.transceiver.mid;
    const meta = (mid != null && this._sfuTrackMap[mid]) || null;
    const ownerId = meta ? meta.userId : null;
    if (!ownerId) return;
    const stream = (event.streams && event.streams[0]) || new MediaStream([track]);
    if (track.kind === 'audio') {
      this._playAudio(ownerId, stream);
    } else {
      // Audio-first v1: forwarded video is best-effort, reusing the same tiles.
      const settings = track.getSettings ? track.getSettings() : {};
      const isWebcam = !settings.displaySurface && this.webcamUsers && this.webcamUsers.has(ownerId);
      if (isWebcam) { if (this.onWebcamStream) this.onWebcamStream(ownerId, stream); }
      else if (this.onScreenStream) this.onScreenStream(ownerId, stream);
    }
  }

  _sfuTeardown() {
    if (this._sfuPc) { try { this._sfuPc.close(); } catch { /* noop */ } this._sfuPc = null; }
    this._sfuTrackMap = {};
    this._sfuMicMids = new Set();
    this.provider = 'p2p';
  }

  _setupSocketListeners() {
    this._setupSfuListeners();
    ...
    this.socket.on('voice-existing-users', async (data) => {
      ...
      // In SFU mode the server relays no per-peer offers — media flows through
      // the single SFU connection, so skip all mesh peer creation here.
      if (this.provider === 'sfu') return;
    ...
    this.socket.on('voice-offer', async (data) => {
      ...
      try {
        ...
        //
        // Both sides of a MSG Arena peer connection can initiate a renegotiation
  ...
  leave() {
    ...
    // Tear down the SFU connection too (no-op on a mesh room).
    this._sfuTeardown();
  ...
  async shareScreen() {
    ...
    if (this.provider === 'sfu') {
      if (this.onSfuFeatureBlocked) this.onSfuFeatureBlocked('screen share');
      return false;
    }
    try {
  ...
  // Toggle 720p vs 480p webcam capture; applies live if a camera is already on.
  async setWebcamHd(on) {
    try { if (typeof localStorage !== 'undefined') localStorage.setItem('haven_webcam_hd', on ? '1' : '0'); } catch {}
    if (this.isWebcamActive && this.webcamStream) {
      const track = this.webcamStream.getVideoTracks()[0];
      if (track && track.applyConstraints) {
        try {
          await track.applyConstraints({
            width:  { ideal: on ? 1280 : 640 },
            height: { ideal: on ? 720 : 480 }
          });
        } catch (e) { /* device may not support it — keep current capture */ }
      }
    }
    return on;
  }

  // Best-effort native background blur; applies live if a camera is on. Returns
  // false when the platform can't do it so the UI can tell the user.
  async setWebcamBlur(on) {
    try { if (typeof localStorage !== 'undefined') localStorage.setItem('haven_webcam_blur', on ? '1' : '0'); } catch {}
    const sc = navigator.mediaDevices.getSupportedConstraints ? navigator.mediaDevices.getSupportedConstraints() : {};
    if (!sc.backgroundBlur) return false;
    if (this.isWebcamActive && this.webcamStream) {
      const track = this.webcamStream.getVideoTracks()[0];
      if (track && track.applyConstraints) {
        try { await track.applyConstraints({ advanced: [{ backgroundBlur: on }] }); }
  ...
  async startWebcam() {
    ...
    if (this.provider === 'sfu') {
      if (this.onSfuFeatureBlocked) this.onSfuFeatureBlocked('webcam');
      return false;
    }
    try {
      const savedCamId = localStorage.getItem('haven_cam_device') || '';
      const hd = (typeof localStorage !== 'undefined' && localStorage.getItem('haven_webcam_hd') === '1');
      const videoConstraints = {
        width:  { ideal: hd ? 1280 : 640 },
        height: { ideal: hd ? 720 : 480 },
        frameRate: { ideal: 30 }
      };
      // Best-effort native background blur where the platform/browser supports it
      // (no ML model, no external deps). Silently ignored where unsupported.
      const blur = (typeof localStorage !== 'undefined' && localStorage.getItem('haven_webcam_blur') === '1');
      const sc = navigator.mediaDevices.getSupportedConstraints ? navigator.mediaDevices.getSupportedConstraints() : {};
      if (blur && sc.backgroundBlur) videoConstraints.backgroundBlur = true;
