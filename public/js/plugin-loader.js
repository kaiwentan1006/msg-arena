/**
 * MSG Arena Plugin & Theme Loader
 * 
 * Plugins:  Drop .plugin.js files into MSG Arena/plugins/
 * Themes:   Drop .theme.css files into MSG Arena/themes/
 * 
 * Plugin format:
...
window.HavenPluginLoader = (function () {
  ...
  async function loadPlugin(meta) {
    ...
    try {
      ...
      if (!PluginClass) {
        console.warn(`[MSG Arena Plugins] Could not find plugin class in ${meta.file}`);
        return;
      }
      ...
      console.error(`[MSG Arena Plugins] Failed to load ${meta.file}:`, err);
    }
  }
  ...
  async function init() {
    try {
      ...
      console.log(`[MSG Arena] Loaded ${loadedPlugins.size} plugin(s), ${loadedThemes.size} theme(s)`);
    } catch (err) {
      console.warn('[MSG Arena] Plugin/theme init error:', err);
    }
  }
