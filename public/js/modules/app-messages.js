  ...
  containerEl.querySelectorAll('.file-video').forEach(video => {
      ...
      try {
        if (!isNaN(video.duration) && video.duration > 0) {
          navigator.mediaSession.metadata = navigator.mediaSession.metadata
            || new MediaMetadata({ title: 'MSG Arena Video' });
    ...
    video.addEventListener('enterpictureinpicture', () => {
      try {
        ...
        navigator.mediaSession.metadata = new MediaMetadata({ title: 'MSG Arena Video' });
  ...
  links.forEach(link => {
    ...
    if (ytVideoId) {
      ...
      wrapper.innerHTML =
        ...
        // referrerpolicy is set per-iframe on purpose. MSG Arena's document-level
    ...
    dataPromise
      .then(data => {
        ...
        if (hasGallery) {
          ...
          // Video is NOT proxied — streaming it through MSG Arena would need Range
...
// Mirrors the MSG Arena mobile app's embed controls. The global Full/Medium/Small/Off
