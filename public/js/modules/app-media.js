...
_setupAvatarUpload() {
  ...
  document.addEventListener('change', (e) => {
    ...
    if (e.target && e.target.id === 'avatar-file-input') {
      ...
      if (!file) return;
      // Must match the server's 2 MB cap (and the "Max 2 MB" hint) — the old 5 MB
      // client limit let 2-5 MB images preview then fail server-side on save (#7).
      if (file.size > 2 * 1024 * 1024) return this._showToast('Avatar must be under 2 MB', 'error');
    ...
    if (e.target && e.target.id === 'border-file-input') {
      const file = e.target.files[0];
      if (!file) return;
      // Match the server's 2 MB cap (see avatar note above) so borders don't
      // preview-then-fail on save (#7).
      if (file.size > 2 * 1024 * 1024) return this._showToast((window.t && window.t('toasts.image_too_large', { max: 2 })) || 'Border must be under 2 MB', 'error');
...
_setupLightbox() {
  ...
  // Zoom/pan state (#5). Reset every time a new image is shown.
  this._lbZoom = 1; this._lbPanX = 0; this._lbPanY = 0;
  this._lbDrag = null;

  // Only close when clicking the backdrop (not the image), and only if the
  // click wasn't the end of a pan drag.
  lb.addEventListener('click', (e) => {
    if (e.target === lb && !this._lbDidPan) this._closeLightbox();
    this._lbDidPan = false;
  });
  document.addEventListener('keydown', (e) => {
    ...
    else if (e.key === '+' || e.key === '=') { e.preventDefault(); this._zoomLightbox(1.4); }
    else if (e.key === '-' || e.key === '_') { e.preventDefault(); this._zoomLightbox(1/1.4); }
    else if (e.key === '0') { e.preventDefault(); this._resetLightboxZoom(); }
  });
  ...
  // Zoom controls
  document.getElementById('lightbox-zoom-in')?.addEventListener('click', (e) => { e.stopPropagation(); this._zoomLightbox(1.4); });
  document.getElementById('lightbox-zoom-out')?.addEventListener('click', (e) => { e.stopPropagation(); this._zoomLightbox(1/1.4); });
  document.getElementById('lightbox-zoom-reset')?.addEventListener('click', (e) => { e.stopPropagation(); this._resetLightboxZoom(); });
  ...
  lb.addEventListener('wheel', (e) => {
    ...
    this._zoomLightbox(e.deltaY < 0 ? 1.18 : 1/1.18, ox, oy);
  }, { passive: false });
  ...
  if (lbImg) {
    ...
    lbImg.addEventListener('dblclick', (e) => {
      e.preventDefault();
      if (this._lbZoom > 1.01) this._resetLightboxZoom();
      else this._zoomLightbox(2.5);
    });
    // Drag to pan when zoomed in
    lbImg.addEventListener('pointerdown', (e) => {
      if (this._lbZoom <= 1.01) return;
      e.preventDefault();
      this._lbDrag = { x: e.clientX, y: e.clientY, px: this._lbPanX, py: this._lbPanY };
      this._lbDidPan = false;
      try { lbImg.setPointerCapture(e.pointerId); } catch {}
    });
    lbImg.addEventListener('pointermove', (e) => {
      if (!this._lbDrag) return;
      const dx = e.clientX - this._lbDrag.x, dy = e.clientY - this._lbDrag.y;
      if (Math.abs(dx) + Math.abs(dy) > 3) this._lbDidPan = true;
      this._lbPanX = this._lbDrag.px + dx;
      this._lbPanY = this._lbDrag.py + dy;
      this._applyLightboxTransform();
    });
    const endDrag = (e) => { if (this._lbDrag) { this._lbDrag = null; try { lbImg.releasePointerCapture(e.pointerId); } catch {} } };
...
// Apply the current zoom + pan to the lightbox image and refresh the label/cursor.
_applyLightboxTransform() {
  const img = document.getElementById('lightbox-img');
  if (!img) return;
  const z = this._lbZoom || 1;
  img.style.transform = `translate(${this._lbPanX || 0}px, ${this._lbPanY || 0}px) scale(${z})`;
  img.style.cursor = z > 1.01 ? (this._lbDrag ? 'grabbing' : 'grab') : 'auto';
  const label = document.getElementById('lightbox-zoom-level');
  if (label) label.textContent = Math.round(z * 100) + '%';
  const resetBtn = document.getElementById('lightbox-zoom-reset');
  if (resetBtn) resetBtn.disabled = z <= 1.01;
},

// Multiply the zoom by `factor`, keeping the point under the cursor stable when
// (ox,oy) offsets from the image centre are supplied. Clamped to [1, 6].
_zoomLightbox(factor, ox = 0, oy = 0) {
  const prev = this._lbZoom || 1;
  let next = Math.min(6, Math.max(1, prev * factor));
  if (next === prev) return;
  // Keep the cursor point anchored: adjust pan by how the offset scales.
  const ratio = next / prev;
  this._lbPanX = (this._lbPanX || 0) * ratio + ox * (1 - ratio);
  this._lbPanY = (this._lbPanY || 0) * ratio + oy * (1 - ratio);
  this._lbZoom = next;
  if (next <= 1.01) { this._lbPanX = 0; this._lbPanY = 0; }
  this._applyLightboxTransform();
},

_resetLightboxZoom() {
  this._lbZoom = 1; this._lbPanX = 0; this._lbPanY = 0; this._lbDrag = null;
  this._applyLightboxTransform();
},
...
_openLightbox(src) {
  ...
  const img = document.getElementById('lightbox-img');
  if (!lb || !img) return;
  // Reparent to <body> so the fixed overlay always lives in the root stacking
  // context. Otherwise, opening it from inside a positioned container (the DM
  // popout, a thread panel) traps it in that container's stacking context and it
  // renders behind the panel — no matter how high its own z-index is.
