...
const LeaderboardMethods = {
  ...
  _renderLeaderboards(data) {
    ...
    if (data.recruiters && data.recruiters.length) {
      sections.push(`<div class="lb-card"><h4>🤝 ${esc(this._lbT('leaderboards.recruiters', 'Top recruiters'))}</h4>
        ${rows(data.recruiters, e => `${e.count} ${esc(e.count === 1 ? this._lbT('leaderboards.recruit_one', 'friend') : this._lbT('leaderboards.recruits', 'friends'))}`)}</div>`);
    }
