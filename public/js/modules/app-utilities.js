...
// emoji-data, so the transform stays correct as MSG Arena's emoji lists grow.
...
_openDMPiP(code) {
  ...
  // Opening a DM focuses the Direct Messages tab so the sidebar stays in sync
  // with what's on screen (#9) — selecting a friend and landing in a DM should
  // light up the DMs pane, not leave you looking at the Channels list.
  try { this._setSidebarPane?.('dms'); } catch {}
  // Ask how far the other person has read our messages so read-ticks are correct
  // the moment the DM opens (#5), not only after they read something new.
  try { this.socket.emit('dm:read-state', { code }); } catch {}
  // Title: partner name
...
_appendDMPiPMessage(msg) {
  ...
  try { this._applyDmReceipts?.(document.getElementById('dm-pip-panel')?.dataset.code); } catch {}
...
// Read receipts (#5): stamp own DM messages with a sent-tick, upgraded to a
// read-tick (accent, double) once the recipient's read position passes them.
// `_dmReadUpTo[code]` is the recipient's last-read message id, from dm:read.
_applyDmReceipts(code) {
  if (!code) return;
  const readUpTo = (this._dmReadUpTo && this._dmReadUpTo[code]) || 0;
  const myId = this.user && this.user.id;
  const lists = [];
  const pip = document.getElementById('dm-pip-panel');
  if (pip && pip.dataset.code === code) { const l = document.getElementById('dm-pip-messages'); if (l) lists.push(l); }
  if (this.currentChannel === code) { const m = document.getElementById('messages'); if (m) lists.push(m); }
  const TICKS = "<svg viewBox='0 0 18 11' width='16' height='11' fill='none' stroke='currentColor' stroke-width='1.7' stroke-linecap='round' stroke-linejoin='round'><path class='tick-1' d='M1 6.2l3.2 3.2L10 2.2'/><path class='tick-2' d='M6.2 6.2l3.2 3.2L15.4 2.2'/></svg>";
  for (const list of lists) {
    list.querySelectorAll('.message, .message-compact').forEach((row) => {
      if (parseInt(row.dataset.userId, 10) !== myId) return;
      const id = parseInt(row.dataset.msgId, 10);
      if (!id) return;
      let r = row.querySelector(':scope .dm-receipt');
      if (!r) {
        r = document.createElement('span');
        r.className = 'dm-receipt';
        r.innerHTML = TICKS;
        (row.querySelector('.message-body') || row).appendChild(r);
      }
      r.classList.toggle('read', id <= readUpTo);
    });
  }
},
...
// One-time identity disclosure on DMs. MSG Arena does not verify who anyone is,
...
_maybeShowDmSafetyNotice(container) {
  ...
  notice.innerHTML =
    ...
    '<span class="dm-safety-text">MSG Arena can\'t verify anyone\'s identity. Names aren\'t unique, so make sure you actually know who you\'re talking to before sharing anything.</span>' +
...
// ═══════════════════════════════════════════════════════
//
// Remote images are fetched by the MSG Arena server and served from its cache, so
...
// and fatal for one that does not: leave MSG Arena open over a weekend and every
...
_confirmTransferAdmin(userId, username) {
  ...
  // An SSO admin has no MSG Arena password, so they confirm with an authenticator
