...
async _checkForUpdates() {
  try {
    // Ask the SERVER for update status. The server checks the single configured
    // release source (UPDATE_REPO in .env) and caches the result, so the repo
    // URL lives in exactly one place and clients don't each hammer GitHub.
    const token = localStorage.getItem('haven_token');
    const res = await fetch('/api/update-status', token ? { headers: { Authorization: `Bearer ${token}` } } : {});
    if (!res.ok) return;
    const data = await res.json();
    const localVersion = data.currentVersion;
    const remoteVersion = data.latestVersion || '';

    if (data.updateAvailable && remoteVersion && localVersion) {
...
async _initDesktopAppBanner() {
  // MSG Arena: the desktop download must point at the operator's OWN source,
  // not the upstream project. Resolve it from the single UPDATE_REPO knob
  // (via /api/update-status → webUrl). If no source is configured, there is
  // nowhere to send people, so the banner stays hidden entirely.
  try {
    const _tok = localStorage.getItem('haven_token');
    const _r = await fetch('/api/update-status', _tok ? { headers: { Authorization: `Bearer ${_tok}` } } : {});
    const _d = _r.ok ? await _r.json() : {};
    const _url = _d.webUrl || _d.url || '';
    if (!_url) {
      document.querySelectorAll('.desktop-app-link, #desktop-promo-install').forEach(a => { a.removeAttribute('href'); });
      const b = document.getElementById('desktop-app-banner');
      if (b) b.style.display = 'none';
      return;
    }
    document.querySelectorAll('.desktop-app-link, #desktop-promo-install').forEach(a => { a.href = _url; });
  } catch { /* leave links as-is on a transient failure */ }
...
_initAndroidBetaBanner() {
  // MSG Arena: the "Amni-Haven Android" promo advertised a third party's
  // Google Play app under upstream branding. It is not an MSG Arena feature,
  // so the banner and its modal are disabled. Kept as a no-op (rather than
  // deleted) so the init call site and the dormant markup stay harmless.
  const _ab = document.getElementById('android-beta-banner');
...
_initWelcomePopups() {
  ...
  // MSG Arena: the intrusive first-login promo popups are gone. The Android
  // one advertised a third party's Play Store app under upstream branding; the
  // desktop one auto-popped a download that pointed at the upstream project.
  // The desktop app stays discoverable via the dismissible top-bar banner
  // (see _initDesktopAppBanner), whose link follows the single UPDATE_REPO
  // source. Keeping this queue machinery in place so future MSG Arena welcome
  // popups can be added here without re-plumbing.
  const allEntries = [];
  void isElectron; void isMobile;
        ...
        try {
          ...
          if (!ok) {
            ...
            if (reason === 'uiohook-unavailable') {
              msg = `Couldn't activate "${accel}" — that bind needs the native input hook (uiohook), which isn't loaded on this machine. Launch MSG Arena from a terminal to see install steps, or pick a regular key combo (e.g. Ctrl+Shift+P) instead.`;
...
async _setupDesktopAppPrefs() {
  ...
  if (versionEl && window.havenDesktop.getVersion) {
    try {
      ...
      versionEl.textContent = `MSG Arena Desktop v${v}`;
    } catch {}
  }
  ...
  sdrEl?.addEventListener('change', async () => {
    try {
      ...
      if (res?.requiresRestart) {
        this._showToast('Color profile updated. Restart MSG Arena Desktop to apply.', 'info');
      }
  ...
  gpuVsyncEl?.addEventListener('change', async () => {
    try {
      ...
      if (res?.requiresRestart) {
        this._showToast?.('GPU vsync setting updated. Restart MSG Arena Desktop to apply.', 'info');
      }
  ...
  unlimitFpsEl?.addEventListener('change', async () => {
    try {
      ...
      if (res?.requiresRestart) {
        this._showToast?.('Frame-rate cap setting updated. Restart MSG Arena Desktop to apply.', 'info');
      }
...
_showE2EPasswordModal() {
  ...
  // (#12) An SSO account has no MSG Arena password — its key is wrapped with the
