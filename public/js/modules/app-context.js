...
async _setupPushNotifications() {
  ...
  // MSG Arena Desktop provides native OS notifications via app-preload.js — hide the web-push section entirely
  ...
  if (!window.isSecureContext) {
    ...
    this._pushErrorReason = 'Push notifications require a secure (HTTPS) connection. Check the MSG Arena setup guide for SSL configuration.';
  ...
  if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
    ...
    if (isIOS && !isStandalone) {
      reason = 'On iOS, push notifications only work when MSG Arena is installed as an app. ' +
        'Tap the Share button → "Add to Home Screen", then open MSG Arena from your home screen.';
    } else if (isIOS) {
  ...
  try {
    ...
    if (err.name === 'SecurityError' || (err.message && err.message.includes('SSL')) || isSelfSigned) {
      reason = 'Push notifications require a trusted SSL certificate.\n\n' +
        ...
        '• Or access MSG Arena via localhost (push works on localhost even with self-signed certs)\n' +
    ...
    if (isBrave) {
      reason = 'Brave blocks push notifications by default.\n\n' +
        ...
        '3. Restart Brave and reload MSG Arena\n\n' +
...
_showPushError(reason) {
  ...
  if (reason.includes('Add to Home Screen')) {
    html += `<div style="margin-top:12px;font-size:0.75rem;color:var(--text-secondary);line-height:1.6;">
      ...
      3. Open MSG Arena from your home screen icon
    </div>`;
  }
  ...
  try {
    ...
    if (isBrave) {
      reason = 'Brave blocked the push subscription.\n\n' +
        ...
        '3. Restart Brave completely (close all windows) and reload MSG Arena\n' +
...
_startPerfDiagnostics() {
  ...
  reportTimer = setInterval(() => {
    ...
    if (samples.length % 4 === 0) {
      console.log(`[MSG Arena Perf] FPS:${avgFps} trend:${trend > 0 ? '+' : ''}${Math.round(trend)} DOM:${domCount}${mem ? ' heap:' + mem.heapUsed + 'MB' : ''} rgb:${isRgbCycling}`);
    }
  }, REPORT_INTERVAL);
