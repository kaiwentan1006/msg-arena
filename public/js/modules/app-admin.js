...
_applyServerSettings() {
  ...
  if (bannerDisplay && bannerImg) {
    if (hasBanner) {
      ...
      // Fit mode: 'cover' (default, fills + crops) or 'contain' (whole banner
      // visible, full opacity, on a neutral backing so letterboxing looks tidy).
      const bannerFit = localStorage.getItem('haven_banner_fit') || 'cover';
      bannerImg.style.objectFit = bannerFit;
      bannerDisplay.classList.toggle('banner-fit-contain', bannerFit === 'contain');
  ...
  const bannerFitSelect = document.getElementById('banner-fit-mode');
  if (bannerFitSelect) bannerFitSelect.value = localStorage.getItem('haven_banner_fit') || 'cover';
...
_applyServerBranding() {
  ...
  if (homeServer) {
    ...
    // A custom logo drops the accent-gradient backing so a transparent logo
    // doesn't show the gradient bleeding through (and a solid one isn't tinted).
    homeServer.classList.toggle('has-custom-logo', !!icon);
...
_initServerBranding() {
  ...
  document.getElementById('banner-fit-mode')?.addEventListener('change', (e) => {
    localStorage.setItem('haven_banner_fit', e.target.value);
    this._applyServerSettings();
    this._showToast(e.target.value === 'contain' ? 'Showing the whole banner' : 'Banner fills the strip', 'success');
  });
...
_renderAllMembers(members) {
  ...
  list.innerHTML = members.map(m => {
    ...
    if (!isSelf(m.id)) {
      ...
      // Admin-only: full user dossier (IPs, country, VPN/proxy, client, IP ban).
      if (perms.isAdmin) {
        btns += `<button class="aml-action-btn aml-btn-details" data-uid="${m.id}" data-uname="${this._escapeHtml(m.username)}" title="User details — IP, country, VPN/proxy, client, IP ban"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><line x1="12" y1="11" x2="12" y2="16"/><circle cx="12" cy="7.6" r="0.7" fill="currentColor" stroke="none"/></svg></button>`;
      }
...
_bindMemberListActions(container) {
  ...
  container.querySelectorAll('.aml-btn-details').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      self._openAdminUserDetails(parseInt(btn.dataset.uid), btn.dataset.uname);
    });
  });
},

// ── Admin-only user dossier: IPs, country, VPN/proxy, client, hard IP ban ──
_openAdminUserDetails(userId, username) {
  if (!this.user || !this.user.isAdmin) return;   // admin only, never mods
  if (!this._adminDetailsWired) {
    this._adminDetailsWired = true;
    this.socket.on('admin:user-details', (d) => this._renderAdminUserDetails(d));
    this.socket.on('admin:ip-intel', (d) => this._fillAdminIpIntel(d));
    const refresh = () => { if (this._adminDetailsUid) this.socket.emit('admin:user-details', { userId: this._adminDetailsUid }); };
    this.socket.on('admin:ip-banned', (d) => { this._showToast && this._showToast('IP banned: ' + d.ip, 'success'); refresh(); });
    this.socket.on('admin:ip-unbanned', (d) => { this._showToast && this._showToast('IP ban lifted: ' + d.ip, 'success'); refresh(); });
  }
  this._adminDetailsUid = userId;
  this.socket.emit('admin:user-details', { userId });
},

_renderAdminUserDetails(d) {
  if (!d || !this.user || !this.user.isAdmin) return;
  const esc = (s) => this._escapeHtml(String(s == null ? '' : s));
  const old = document.getElementById('admin-user-details-modal');
  if (old) old.remove();
  const created = d.createdAt ? new Date(d.createdAt).toLocaleString() : '—';
  const ipRows = (d.ips || []).map((ip, i) => `
    <div class="aud-ip-row" data-ip="${esc(ip.ip)}">
      <div class="aud-ip-line">
        <span class="aud-ip-addr">${esc(ip.ip)}</span>
        ${ip.banned ? '<span class="aud-badge aud-badge-danger">IP banned</span>' : ''}
        <span class="aud-ip-intel" id="aud-intel-${i}"></span>
      </div>
      <div class="aud-ip-actions">
        <button class="btn-sm aud-lookup" data-ip="${esc(ip.ip)}" data-slot="aud-intel-${i}">Look up</button>
        ${ip.banned
          ? `<button class="btn-sm aud-unban" data-ip="${esc(ip.ip)}">Unban IP</button>`
          : `<button class="btn-sm btn-danger-fill aud-ban" data-ip="${esc(ip.ip)}">Ban IP</button>`}
      </div>
    </div>`).join('') || '<p class="muted-text">No recorded IP addresses yet.</p>';
  ...
  modal.innerHTML = `
    <div class="modal aud-box" style="max-width:560px">
      <div class="modal-header">
        <h4>User details — ${esc(d.displayName)} <span class="muted-text" style="font-weight:normal">@${esc(d.username)}</span></h4>
        <button class="modal-close-btn aud-close" aria-label="Close">&times;</button>
      </div>
      <div class="modal-body">
        <div class="aud-grid">
          <div class="aud-cell"><span class="aud-k">Status</span><span class="aud-v">${d.online ? 'Online' : 'Offline'}${d.banned ? ' · <span class="aud-badge aud-badge-danger">Account banned</span>' : ''}</span></div>
          <div class="aud-cell"><span class="aud-k">Client</span><span class="aud-v">${esc(d.client)}</span></div>
          <div class="aud-cell"><span class="aud-k">Role</span><span class="aud-v">${d.isAdmin ? 'Admin' : 'Member'}</span></div>
          <div class="aud-cell"><span class="aud-k">Joined</span><span class="aud-v">${esc(created)}</span></div>
        </div>
        <div class="aud-section-label">IP addresses (5 most recent)</div>
        <p class="settings-hint" style="margin:0 0 10px">Look up an address to see its country and whether it's a VPN / proxy / datacenter, then ban the genuine one. A VPN can't be fully unmasked from the server — banning the address (and dropping its live sessions) is the reliable defence; pair it with a registration token or captcha to slow re-entry.</p>
        <div class="aud-ips">${ipRows}</div>
      </div>
    </div>`;
  document.body.appendChild(modal);
  const close = () => modal.remove();
  modal.querySelector('.aud-close').addEventListener('click', close);
  modal.addEventListener('click', (e) => { if (e.target === modal) close(); });
  this._adminIntelSlot = this._adminIntelSlot || {};
  modal.querySelectorAll('.aud-lookup').forEach(b => b.addEventListener('click', () => {
    const slot = document.getElementById(b.dataset.slot);
    if (slot) slot.textContent = ' …';
    this._adminIntelSlot[b.dataset.ip] = b.dataset.slot;
    this.socket.emit('admin:ip-intel', { ip: b.dataset.ip });
  }));
  modal.querySelectorAll('.aud-ban').forEach(b => b.addEventListener('click', () => {
    if (window.confirm('Ban ' + b.dataset.ip + '? Anyone connecting from this address will be blocked.'))
      this.socket.emit('admin:ban-ip', { ip: b.dataset.ip, reason: 'Linked to @' + d.username });
  }));
  modal.querySelectorAll('.aud-unban').forEach(b => b.addEventListener('click', () => {
    this.socket.emit('admin:unban-ip', { ip: b.dataset.ip });
  }));
},

_fillAdminIpIntel(d) {
  if (!d) return;
  const slotId = (this._adminIntelSlot || {})[d.ip];
  const slot = slotId && document.getElementById(slotId);
  if (!slot) return;
  const intel = d.intel;
  if (!intel) { slot.textContent = ' (lookup failed)'; return; }
  // Private/CGNAT/loopback address → the real public IP wasn't captured.
  if (intel.private) {
    slot.innerHTML = ' <span class="aud-badge aud-badge-warn">Private / not routable</span>'
      + `<span class="aud-intel-note">${this._escapeHtml(intel.note || 'The real public IP was not captured behind the proxy.')}</span>`;
    return;
  }
  if (intel.error) { slot.innerHTML = ` <span class="aud-badge">Couldn't geolocate (${this._escapeHtml(intel.error)})</span>`; return; }
  const b = [];
  const place = [intel.city, intel.region, (intel.country || intel.countryCode)].filter(Boolean).join(', ');
  b.push(`<span class="aud-badge">${this._escapeHtml(place || 'Unknown location')}</span>`);
  if (intel.proxy) b.push('<span class="aud-badge aud-badge-danger">VPN / Proxy</span>');
  if (intel.hosting) b.push('<span class="aud-badge aud-badge-warn">Datacenter</span>');
  if (intel.mobile) b.push('<span class="aud-badge">Mobile network</span>');
  if (!intel.proxy && !intel.hosting) b.push('<span class="aud-badge aud-badge-ok">Looks residential</span>');
  if (intel.isp) b.push(`<span class="aud-intel-note">${this._escapeHtml(intel.isp)}</span>`);
  if (intel.proxy) b.push('<span class="aud-intel-note">Shown country is the VPN/proxy exit, not the user’s real location (which can’t be revealed server-side). Ban this address to cut it off.</span>');
  slot.innerHTML = ' ' + b.join(' ');
},
...
_setupLevelRoles() {
  const addBtn = document.getElementById('level-role-add');
  const roleSel = document.getElementById('level-role-role');
  if (!addBtn || !roleSel) return;
  this._populateLevelRoleSelect = () => {
    const roles = (this._allRoles || []).filter(r => r.scope !== 'channel');
    const cur = roleSel.value;
    roleSel.innerHTML = roles.length
      ? roles.map(r => `<option value="${r.id}">${this._escapeHtml(r.name)}</option>`).join('')
      : `<option value="">${t('settings.admin.level_roles_need_role', {}, 'Create a role first')}</option>`;
  ...
  if (!addBtn._wired) {
    ...
    addBtn.addEventListener('click', () => {
      const level = parseInt(document.getElementById('level-role-level').value, 10);
      const roleId = parseInt(roleSel.value, 10);
      if (!(level > 0) || !(roleId > 0)) return this._showToast(t('settings.admin.level_roles_pick', {}, 'Pick a level and a role'), 'error');
      this._roleEmit('xp:set-level-role', { level, roleId }, (res) => {
        if (!res || res.error) return this._showToast((res && res.error) || 'Error', 'error');
        this._renderLevelRolesList(res.levelRoles);
        this._showToast(res.granted
          ? t('settings.admin.level_roles_set_n', { n: res.granted }, `Reward set — gave it to ${res.granted} member(s)`)
          : t('settings.admin.level_roles_set', {}, 'Reward set'), 'success');
      });
    });
  }
  this._roleEmit('xp:list-level-roles', {}, (res) => this._renderLevelRolesList(res && res.levelRoles));
},

_renderLevelRolesList(list) {
  const host = document.getElementById('level-roles-list');
  if (!host) return;
  list = Array.isArray(list) ? list.slice().sort((a, b) => a.level - b.level) : [];
  if (!list.length) {
    host.innerHTML = `<span class="settings-hint" style="opacity:.65">${t('settings.admin.level_roles_empty', {}, 'No level rewards yet.')}</span>`;
    return;
  }
  host.innerHTML = list.map(lr =>
    `<div class="level-role-row" style="display:flex;align-items:center;gap:8px">
       <span class="gs-level-badge" style="min-width:58px;text-align:center">Lv ${lr.level}</span>
       <span style="flex:1;font-weight:600;color:${/^#[0-9a-fA-F]{3,6}$/.test(lr.color || '') ? lr.color : 'var(--text-primary)'}">${lr.icon ? this._escapeHtml(lr.icon) + ' ' : ''}${this._escapeHtml(lr.name)}</span>
       <button class="btn-sm" data-lr-remove="${lr.level}">${t('settings.admin.level_roles_remove', {}, 'Remove')}</button>
     </div>`).join('');
  host.querySelectorAll('[data-lr-remove]').forEach(b => b.addEventListener('click', () => {
    const level = parseInt(b.getAttribute('data-lr-remove'), 10);
    this._roleEmit('xp:remove-level-role', { level }, (res) => {
      if (res && res.error) return this._showToast(res.error, 'error');
      this._renderLevelRolesList(res && res.levelRoles);
    });
  }));
},
...
_loadRoles(cb) {
  this._roleEmit('get-roles', {}, (res) => {
    ...
    if (this._populateLevelRoleSelect) this._populateLevelRoleSelect();
