...
// ── First-run onboarding tour (Discord-style coach marks) ──────────────
// Highlights the core controls one at a time with a "Next" / "Skip all" popover.
// Runs once for new users (localStorage flag); pass force=true to replay it.
_startOnboardingTour(force) {
  try { if (!force && localStorage.getItem('ma_onboarding_done')) return; } catch {}
  if (document.querySelector('.tour-overlay')) return;
  const steps = [
    { sel: '#rail-avatar-btn',    title: 'Your account',        text: 'Click your avatar any time for your profile, Rank Rewards, settings and logout.' },
    { sel: '.sidebar-tabs',       title: 'Channels & DMs',      text: 'Switch between the server’s channels and your direct messages here.' },
    { sel: '#channel-code-input', title: 'Join a channel',      text: 'Got an invite code from a friend? Paste it here and hit Join.' },
    { sel: '#games-btn',          title: 'Find players',        text: 'Find teammates for your games by rank, region and language.' },
    { sel: '#lfg-btn',            title: 'Looking for group',   text: 'Post an LFG when you need people for a match — a voice room opens automatically when it fills.' },
    { sel: '.sidebar-bottom-bar',      title: 'Your gamer toolkit',  text: 'Tournaments, clips, leaderboards, events and more all live down here.' },
    { sel: '#right-btn-invite-popout', title: 'Invite your friends', text: 'Share an invite link to bring your whole squad in.' },
    { sel: '#voice-join-btn',          title: 'Jump into voice',     text: 'One click drops you into low-latency voice with everyone in the channel.' },
  ];
  const visible = (el) => el && el.offsetParent !== null && el.getBoundingClientRect().width > 0;
  const live = steps.filter(s => visible(document.querySelector(s.sel)));
  if (!live.length) { try { localStorage.setItem('ma_onboarding_done', '1'); } catch {} return; }
  ...
  let i = 0;
  const esc = (t) => this._escapeHtml ? this._escapeHtml(t) : t;
  const end = () => {
    overlay.remove(); pop.remove();
    window.removeEventListener('resize', render);
    try { localStorage.setItem('ma_onboarding_done', '1'); } catch {}
  };
  const next = () => { i++; if (i >= live.length) return end(); render(); };
  function render() {
    const s = live[i];
    const el = document.querySelector(s.sel);
    if (!visible(el)) return next();
    const r = el.getBoundingClientRect(); const pad = 8;
    spot.style.left = (r.left - pad) + 'px'; spot.style.top = (r.top - pad) + 'px';
    spot.style.width = (r.width + pad * 2) + 'px'; spot.style.height = (r.height + pad * 2) + 'px';
    pop.innerHTML =
      `<h4>${esc(s.title)}</h4><p>${esc(s.text)}</p>
       <div class="tour-pop-foot"><span class="tour-step-count">${i + 1} / ${live.length}</span>
       <div class="tour-actions"><button class="tour-skip">Skip all</button>
       <button class="tour-next">${i === live.length - 1 ? 'Done' : 'Next'}</button></div></div>`;
    const pw = 300, ph = pop.offsetHeight || 150; const gap = 14;
    let px = r.right + gap, py = r.top;
    if (px + pw > window.innerWidth - 10) { px = r.left; py = r.bottom + gap; }      // no room right → below
    if (px + pw > window.innerWidth - 10) px = window.innerWidth - pw - 10;
    if (px < 10) px = 10;
    if (py + ph > window.innerHeight - 10) py = Math.max(10, window.innerHeight - ph - 10);
...
// Mirror the total DM unread count onto the DM tab badge and the
// missed-messages banner above the tabs (shown only when not already on DMs).
_updateDmTabBadge(count) {
  this._dmUnreadTotal = count || 0;
  const split = document.getElementById('sidebar-split');
  const onDms = split && split.getAttribute('data-pane') === 'dms';
  const badge = document.getElementById('dm-tab-badge');
  if (badge) {
    // The aggregate tab badge is a "there's new DM activity" cue. While the DM
    // pane is the one on screen it's redundant (the per-conversation unread dots
    // carry the detail), so hide it — that's what makes it vanish once you look.
    if (count > 0 && !onDms) { badge.textContent = count > 99 ? '99+' : count; badge.style.display = ''; }
    else badge.style.display = 'none';
  }
  const banner = document.getElementById('dm-missed-banner');
  if (banner) {
    if (count > 0 && !onDms) {
      banner.textContent = count === 1 ? '1 missed message' : `${count} missed messages`;
...
// Switch the sidebar between the Channels and Direct Messages panes. Kept as an
// instance method (not a closure) so other flows can drive it — e.g. opening a
// DM focuses the DMs tab (#9). Re-evaluates both tab badges so the pane you just
// moved to counts as "seen".
_setSidebarPane(pane) {
  const split = document.getElementById('sidebar-split');
  const tabChannels = document.getElementById('tab-channels');
  const tabDms = document.getElementById('tab-dms');
  if (!split || !tabChannels || !tabDms) return;
  split.setAttribute('data-pane', pane);
  tabChannels.classList.toggle('active', pane === 'channels');
  tabDms.classList.toggle('active', pane === 'dms');
  try { localStorage.setItem('ma_sidebar_pane', pane); } catch {}
  this._updateChannelsTabBadge(this._chUnreadTotal || 0);
  this._updateDmTabBadge(this._dmUnreadTotal || 0);
},

// Discover public servers from the opt-in directory (central registry). Opening a
// server takes you to that separate deploy.
_openServerDirectory() {
  const existing = document.getElementById('server-directory-modal');
  if (existing) existing.remove();
  const esc = (s) => this._escapeHtml ? this._escapeHtml(String(s == null ? '' : s)) : String(s);
  const modal = document.createElement('div');
  modal.id = 'server-directory-modal';
  modal.className = 'modal-overlay';
  modal.style.display = 'flex';
  modal.style.zIndex = '100002';
  modal.innerHTML = `
    <div class="modal" style="max-width:560px;max-height:82vh;display:flex;flex-direction:column">
      <div class="modal-header"><h4>Discover servers</h4><button class="modal-close-btn sd-close" aria-label="Close">&times;</button></div>
      <div class="modal-body" style="display:flex;flex-direction:column;min-height:0;flex:1">
        <input type="text" id="sd-search" class="settings-text-input" placeholder="Search servers…" style="margin-bottom:10px">
        <p class="settings-hint" style="margin:0 0 8px">Public MSG Arena servers that opted into the directory. Opening one takes you to that server.</p>
        <div id="sd-list" style="overflow-y:auto;flex:1;min-height:120px;display:flex;flex-direction:column;gap:8px">
          <p class="muted-text" style="text-align:center;padding:16px">Loading…</p>
        </div>
      </div>
    </div>`;
  document.body.appendChild(modal);
  const close = () => modal.remove();
  modal.querySelector('.sd-close').addEventListener('click', close);
  modal.addEventListener('click', (e) => { if (e.target === modal) close(); });
  const listEl = modal.querySelector('#sd-list');
  const render = (servers) => {
    if (!servers || !servers.length) { listEl.innerHTML = '<p class="muted-text" style="text-align:center;padding:16px">No servers are listed yet.</p>'; return; }
    listEl.innerHTML = servers.map((s) => `
      <div class="sd-item">
        <div class="sd-item-info">
          <div class="sd-item-name">${esc(s.name)}</div>
          ${s.description ? `<div class="sd-item-desc">${esc(s.description)}</div>` : ''}
          <div class="sd-item-url">${esc(s.url)}</div>
        </div>
        <button class="btn-sm btn-accent sd-open" data-url="${esc(s.url)}">Open</button>
      </div>`).join('');
    listEl.querySelectorAll('.sd-open').forEach((b) => b.addEventListener('click', () => window.open(b.dataset.url, '_blank', 'noopener')));
  };
  const load = (q) => {
    listEl.innerHTML = '<p class="muted-text" style="text-align:center;padding:16px">Loading…</p>';
    fetch('/api/registry/servers' + (q ? ('?q=' + encodeURIComponent(q)) : ''))
      .then((r) => r.json())
      .then((j) => render(j.servers || []))
      .catch(() => { listEl.innerHTML = '<p class="muted-text" style="text-align:center;padding:16px">Directory unavailable.</p>'; });
  };
  let deb;
  modal.querySelector('#sd-search').addEventListener('input', (e) => { clearTimeout(deb); const q = e.target.value; deb = setTimeout(() => load(q), 250); });
  load('');
},

// Unread total for channels → badge on the Channels tab (so you notice channel
// activity while you're on the DMs tab, and vice-versa with the DM badge).
_updateChannelsTabBadge(count) {
  this._chUnreadTotal = count || 0;
  const badge = document.getElementById('channels-tab-badge');
  if (!badge) return;
  const split = document.getElementById('sidebar-split');
  const onChannels = split && split.getAttribute('data-pane') === 'channels';
  // Hidden while the Channels pane is showing (you're already looking at it).
  if (count > 0 && !onChannels) { badge.textContent = count > 99 ? '99+' : count; badge.style.display = ''; }
  else badge.style.display = 'none';
},
...
_setupUI() {
  ...
  // ── Webcam HD (720p) toggle ──
  const wcHdBtn = document.getElementById('webcam-hd-btn');
  if (wcHdBtn) {
    let hdOn = localStorage.getItem('haven_webcam_hd') === '1';
    wcHdBtn.classList.toggle('active', hdOn);
    wcHdBtn.addEventListener('click', () => {
      hdOn = !hdOn;
      wcHdBtn.classList.toggle('active', hdOn);
      if (this.voice && this.voice.setWebcamHd) this.voice.setWebcamHd(hdOn);
      else { try { localStorage.setItem('haven_webcam_hd', hdOn ? '1' : '0'); } catch {} }
      if (this._showToast) this._showToast(hdOn ? t('media.hd_on') : t('media.hd_off'), 'info');
    });
  }

  // ── Webcam background-blur toggle (best-effort native; shown only if supported) ──
  const wcBlurBtn = document.getElementById('webcam-blur-btn');
  if (wcBlurBtn) {
    const sc = (navigator.mediaDevices && navigator.mediaDevices.getSupportedConstraints)
      ? navigator.mediaDevices.getSupportedConstraints() : {};
    if (sc.backgroundBlur) {
      wcBlurBtn.hidden = false;
      let blurOn = localStorage.getItem('haven_webcam_blur') === '1';
      wcBlurBtn.classList.toggle('active', blurOn);
      wcBlurBtn.setAttribute('aria-pressed', blurOn ? 'true' : 'false');
      wcBlurBtn.addEventListener('click', () => {
        blurOn = !blurOn;
        wcBlurBtn.classList.toggle('active', blurOn);
        wcBlurBtn.setAttribute('aria-pressed', blurOn ? 'true' : 'false');
        if (this.voice && this.voice.setWebcamBlur) this.voice.setWebcamBlur(blurOn);
        else { try { localStorage.setItem('haven_webcam_blur', blurOn ? '1' : '0'); } catch {} }
        if (this._showToast) this._showToast(blurOn ? t('media.blur_on') : t('media.blur_off'), 'info');
      });
    }
  }
  ...
  // In SFU (large-room) mode, screen share / webcam aren't available yet.
  this.voice.onSfuFeatureBlocked = (feature) =>
    this._showToast((t('voice.sfu_feature_blocked', { feature }) || `${feature} isn't available in large (SFU) voice rooms yet`), 'error');
  ...
  document.addEventListener('keydown', (e) => {
    ...
    // jumping the main channel behind them is never what was meant. MSG Arena's
  ...
  const dmPipNudge = document.getElementById('dm-pip-nudge');
  if (dmPipNudge) dmPipNudge.addEventListener('click', () => {
    const code = this._activeDMPip;
    if (!code) return;
    this.socket.emit('dm:nudge', { code });
    try { navigator.vibrate && navigator.vibrate(25); } catch {}   // tiny confirmation buzz
  });
  ...
  if (dmPipInput) dmPipInput.addEventListener('paste', (e) => {
    ...
    for (const f of files) {
      if (f.type.startsWith('image/')) this._queueImageForPiP(f, targetCode);
      else this._uploadGeneralFile(f, targetCode);
    }
  });
  ...
  if (threadInput) {
    ...
    threadInput.addEventListener('paste', (e) => {
      ...
      if (!files.length) return;
      e.preventDefault();
      // Hold them, don't post. Flushed on send. (#thread-paste-instant)
      for (const f of files) this._queueThreadFile(f);
    });
    ...
    threadArea.addEventListener('drop', (e) => {
      ...
      if (files) for (const f of files) this._queueThreadFile(f);
    });
  }
  ...
  const openSettingsModal = () => {
    ...
    // Show desktop-only sections when running inside MSG Arena Desktop
  ...
  // ── Account avatar pinned to the server rail ─────────────────────────
  // Replaces the top user-bar + the standalone Settings button. The avatar
  // shows the user's picture (or initials) with a status badge; clicking it
  // opens an account menu whose items just drive the existing controls.
  const railBtn = document.getElementById('rail-avatar-btn');
  if (railBtn) {
    ...
    const railName   = document.getElementById('rail-menu-name');
    const railUser   = document.getElementById('rail-menu-username');
    const updateRail = () => {
      const u = this.user || {};
      const name = u.displayName || u.username || '';
      if (railName) railName.textContent = name;
      if (railUser) railUser.textContent = u.username ? '@' + u.username : '';
      if (railAvatar) {
        if (u.avatar) {
          railAvatar.innerHTML = `<img src="${this._escapeHtml ? this._escapeHtml(u.avatar) : u.avatar}" alt="">`;
          railAvatar.classList.add('has-img');
        } else {
          railAvatar.textContent = (name.trim()[0] || '?').toUpperCase();
          railAvatar.classList.remove('has-img');
        }
      }
      if (railDot) railDot.className = 'rail-status-dot ' + (u.status || 'online');
    };
    updateRail();
    // Keep the rail fresh after a rename (the app writes the name into #current-user).
    const cu = document.getElementById('current-user');
    if (cu) new MutationObserver(updateRail).observe(cu, { childList: true, characterData: true, subtree: true });
    const closeRailMenu = () => { railMenu.style.display = 'none'; railBtn.classList.remove('open'); };
    railBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      if (railMenu.style.display !== 'none') { closeRailMenu(); return; }
      updateRail();
      // Move to <body> so it lives in the root stacking context (the rail sits in
      // a lower one, so an in-place menu renders behind the sidebar) and its fixed
      // position is never clipped by the rail's overflow.
      if (railMenu.parentElement !== document.body) document.body.appendChild(railMenu);
      const r = railBtn.getBoundingClientRect();
      railMenu.style.left = (r.right + 8) + 'px';
      railMenu.style.bottom = (window.innerHeight - r.bottom) + 'px';
      railMenu.style.display = 'block';
      railBtn.classList.add('open');
    });
    document.addEventListener('click', (e) => {
      if (railMenu.style.display !== 'none' && !railMenu.contains(e.target) && !railBtn.contains(e.target)) closeRailMenu();
    });
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape') closeRailMenu(); });
    const wireRail = (id, targetId) => {
      document.getElementById(id)?.addEventListener('click', () => {
        closeRailMenu();
        document.getElementById(targetId)?.click();
      });
    };
    wireRail('rail-menu-settings', 'open-settings-btn');
    wireRail('rail-menu-profile',  'rename-btn');   // opens the combined Edit Profile modal
    wireRail('rail-menu-logout',   'logout-btn');
    document.getElementById('rail-menu-tour')?.addEventListener('click', () => {
      closeRailMenu();
      this._startOnboardingTour(true);
    });
    document.getElementById('rail-menu-download')?.addEventListener('click', () => {
      closeRailMenu();
      // The /download page lists desktop (Windows/Linux) + Android builds.
      window.open('/download', '_blank', 'noopener');
    });
  }

  // "Browse public servers" from the Add-Server modal → the directory.
  document.getElementById('browse-directory-btn')?.addEventListener('click', () => {
    const asm = document.getElementById('add-server-modal');
    if (asm) asm.style.display = 'none';
    this._openServerDirectory();
  });

  // ── Channels / Direct Messages tabs (one pane at a time) ─────────────
  const sidebarSplit = document.getElementById('sidebar-split');
  const tabChannels = document.getElementById('tab-channels');
  const tabDms = document.getElementById('tab-dms');
  if (sidebarSplit && tabChannels && tabDms) {
    tabChannels.addEventListener('click', () => this._setSidebarPane('channels'));
    tabDms.addEventListener('click', () => this._setSidebarPane('dms'));
    const banner = document.getElementById('dm-missed-banner');
    if (banner) {
      const goDms = () => this._setSidebarPane('dms');
      banner.addEventListener('click', goDms);
      banner.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); goDms(); } });
    }
    let savedPane = 'channels';
    try { savedPane = localStorage.getItem('ma_sidebar_pane') || 'channels'; } catch {}
    this._setSidebarPane(savedPane);
  }

  // First-run onboarding tour: once the app has settled and no first-run modal is
  // covering the controls, walk new users through the core features (Discord-style).
  try {
    if (!localStorage.getItem('ma_onboarding_done')) {
      setTimeout(() => {
        const modalUp = [...document.querySelectorAll('.modal-overlay')].some(m => getComputedStyle(m).display !== 'none');
        if (modalUp) setTimeout(() => this._startOnboardingTour(), 5000);
        else this._startOnboardingTour();
      }, 2800);
    }
  } catch {}
  ...
  this._switchSettingsTab = (tab) => {
    if (this._clearSettingsSearch) this._clearSettingsSearch();
  ...
  // 44+ settings across two tabs read as a wall. A live filter over BOTH the nav
  // and the section bodies turns "scroll and hope" into "type what you want".
  // Uses a hide CLASS (not inline display) so it never fights the per-permission
  // visibility the admin code sets — it only filters what is already visible.
  (() => {
    ...
    const activeBody = () => {
      const a = document.getElementById('settings-body-admin');
      return (a && a.style.display !== 'none') ? a : document.getElementById('settings-body-user');
    };
    const activeNav = () => {
      const a = document.querySelector('.settings-nav-admin-group');
      return (a && a.style.display !== 'none') ? a : document.querySelector('.settings-nav-user');
    };
    const apply = () => {
      const q = norm(input.value.trim());
      if (clearBtn) clearBtn.hidden = !q;
      const body = activeBody(), nav = activeNav();
      if (!body || !nav) return;
      if (!q) {
        document.querySelectorAll('.settings-search-hidden').forEach(e => e.classList.remove('settings-search-hidden'));
        if (emptyMsg) emptyMsg.hidden = true;
        this._settingsSearchActive = false;
        return;
      }
      this._settingsSearchActive = true;
      this._settingsSpyMuteUntil = Date.now() + 2000;
      let any = false;
      const matched = new Set();
      body.querySelectorAll('.settings-section, .admin-settings').forEach(sec => {
        const hit = norm(sec.textContent).includes(q);
        sec.classList.toggle('settings-search-hidden', !hit);
        if (hit) { any = true; if (sec.id) matched.add(sec.id); }
      });
      nav.querySelectorAll('.settings-nav-item').forEach(it => {
        const labelHit = norm(it.textContent).includes(q);
        const show = labelHit || matched.has(it.dataset.target);
        it.classList.toggle('settings-search-hidden', !show);
        if (labelHit && it.dataset.target) {
          const sec = document.getElementById(it.dataset.target);
          if (sec) { sec.classList.remove('settings-search-hidden'); any = true; }
        }
      });
      // Hide group labels whose whole group filtered out.
      nav.querySelectorAll('.settings-nav-group-label').forEach(label => {
        let el = label.nextElementSibling, visible = false;
        while (el && !el.classList.contains('settings-nav-group-label')) {
          if (el.classList.contains('settings-nav-item') && !el.classList.contains('settings-search-hidden') && el.style.display !== 'none') visible = true;
          el = el.nextElementSibling;
        }
        label.classList.toggle('settings-search-hidden', !visible);
      });
      if (emptyMsg) emptyMsg.hidden = any;
      const first = body.querySelector('.settings-section:not(.settings-search-hidden), .admin-settings:not(.settings-search-hidden)');
      if (first) body.scrollTop = Math.max(0, first.offsetTop - 8);
    };
    ...
    this._clearSettingsSearch = () => { if (input.value) { input.value = ''; apply(); } };
  })();
  ...
  const startBackupDownload = async (include) => {
    const token = localStorage.getItem('haven_token');
    if (!token) return this._showToast(t('toasts.not_logged_in') || 'Not logged in', 'error');
    // Fetch a short-lived, download-only token over the authenticated API so
    // the full session JWT never lands in the URL (and thus in logs/history).
    let dt;
    try {
      const r = await fetch('/api/admin/download-token', { headers: { 'Authorization': `Bearer ${token}` } });
      if (!r.ok) throw new Error(`HTTP ${r.status}`);
      dt = (await r.json()).token;
    } catch (e) {
      return this._showToast(t('toasts.backup_restore_failed') || 'Could not start backup', 'error');
    }
    if (!dt) return this._showToast(t('toasts.backup_restore_failed') || 'Could not start backup', 'error');
    const url = `/api/admin/backup?include=${encodeURIComponent(include)}&dt=${encodeURIComponent(dt)}`;
    ...
    try {
      ...
      if (status) {
        ...
        status.innerHTML = `
          ...
          <div style="margin-top:6px">${data.notConfigured ? '⚙️ Update checks are off (set UPDATE_REPO in .env).' : (upToDate ? '✅ You are up to date.' : '⚠️ Update available.')}</div>
...
/** Copy a MSG Arena-style deep link to a channel (and optionally a message) to the clipboard. */
 ...
 *  OTHER MSG Arena servers the next time the user switches. */
...
_setupServerBar() {
  // Launcher dock: turn a vertical mouse wheel into smooth horizontal scroll,
  // so the single-row dock feels natural to browse (CSS does the fade + smooth).
  const _dock = document.querySelector('.sidebar-bottom-bar');
  if (_dock && !_dock._wheelWired) {
    _dock._wheelWired = true;
    _dock.addEventListener('wheel', (e) => {
      if (!e.deltaY || _dock.scrollWidth <= _dock.clientWidth) return;
      e.preventDefault();
      _dock.scrollBy({ left: e.deltaY, behavior: 'smooth' });
    }, { passive: false });
  }
...
_addServerFromInvite() {
  // Opening an invite link is how you join a server here, so it should also
  // REMEMBER that server in your rail — no manual "Add Server". Adds the current
  // origin (name/icon from its public /api/health) and syncs it exactly like a
  // manual add. userInitiated:true because opening the invite is deliberate, so
  // it overrides the removed-set. Non-fatal on any error — the join still worked.
  (async () => {
    try {
      if (!this.serverManager) return;
      const origin = window.location.origin;
      let name = null, icon = null;
      try {
        const r = await fetch('/api/health', { cache: 'no-store' });
        if (r.ok) { const d = await r.json(); name = d.name || null; icon = d.icon || null; }
      } catch { /* health ping optional */ }
      name = name || (location.hostname || origin);
      if (this.serverManager.add(name, origin, icon, { userInitiated: true })) {
        this._renderServerBar?.();
        this._pushServerListToServer?.();
        this._pushServersToDesktopHistory?.();
        if (window.havenDesktop?.addServerHistory) window.havenDesktop.addServerHistory(origin, name).catch(() => {});
        this._showToast?.(t('toasts.server_added_from_invite', { name }, `Added ${name} to your servers`), 'success');
      }
    } catch { /* non-fatal */ }
  })();
},
...
_addServer() {
  let name = document.getElementById('add-server-name-input').value.trim();
  let url = document.getElementById('server-url-input').value.trim();
  // People share invite / app links, not bare origins. Accept a pasted link
  // (…/?invite=CODE, …/invite/CODE, …/app) by reducing it to the server origin,
  // and auto-name from the host if the name field was left blank.
  if (/^https?:\/\//i.test(url)) { try { const u = new URL(url); url = u.origin; if (!name) name = u.hostname; } catch {} }
...
_setupImageUpload() {
  ...
  uploadBtn.addEventListener('click', () => {
    ...
    try { fileInput.multiple = true; } catch {}  // allow picking several files at once
    fileInput.click();
  });
  ...
  fileInput.addEventListener('change', () => {
    ...
    for (const file of fileInput.files) {
      if (file.type.startsWith('image/')) this._queueImage(file);
  ...
  document.getElementById('message-input').addEventListener('paste', (e) => {
    const items = e.clipboardData?.items;
    if (!items) return;
    // Collect EVERY pasted file, not just the first — pasting several files at
    // once must queue all of them. getAsFile() must run synchronously here.
    const files = [];
    ...
    e.preventDefault();
    for (const f of files) {
      if (f.type.startsWith('image/')) this._queueImage(f);
      else this._queueGeneralFile(f);
    }
  });
  ...
  messageArea.addEventListener('drop', (e) => {
    ...
    // Queue every dropped file, not just the first.
...
_setupMobile() {
  ...
  // Sticky sender avatar for long same-author runs (#11).
  this._initStickyAvatars();
},

// ── Sticky sender avatar (#11) ──────────────────────────────────────────
// When one person posts a run of messages taller than the viewport, their
// avatar (shown once on the run's first message) scrolls away, so you lose
// track of who's talking. Rather than restructure the flat message DOM into
// per-author group wrappers (which would ripple through create/promote/delete/
// scroll-anchoring), we float a copy of the run's avatar pinned to the top of
// the message area — it appears when the real avatar scrolls off and slides out
// as the next author's run reaches the top.
_initStickyAvatars() {
  if (this._stickyAvatarsInit) return;
  this._stickyAvatarsInit = true;
  const specs = [
    { id: 'messages', pad: 10 },
    { id: 'dm-pip-messages', pad: 8 },
    { id: 'thread-messages', pad: 8 },
  ];
  for (const spec of specs) {
    const container = document.getElementById(spec.id);
    if (!container) continue;
    const floater = document.createElement('div');
    floater.className = 'sticky-avatar';
    floater.style.display = 'none';
    document.body.appendChild(floater);
    let ticking = false;
    const run = () => { ticking = false; this._updateStickyAvatar(container, floater, spec.pad); };
    const onChange = () => { if (!ticking) { ticking = true; requestAnimationFrame(run); } };
    container.addEventListener('scroll', onChange, { passive: true });
    window.addEventListener('resize', onChange, { passive: true });
    // Content replaced (channel switch) or grown (new messages) → re-evaluate.
    try { new MutationObserver(onChange).observe(container, { childList: true, subtree: false }); } catch {}
  }
},

_updateStickyAvatar(container, floater, pad) {
  // Hidden container (DM pip closed, thread closed, off-screen) → nothing to pin.
  if (!container.isConnected || container.offsetParent === null) { floater.style.display = 'none'; return; }
  const cRect = container.getBoundingClientRect();
  if (cRect.height < 40) { floater.style.display = 'none'; return; }
  const topEdge = cRect.top + pad;

  // First message row still (partly) visible at the top edge.
  const rows = container.querySelectorAll(':scope > .message, :scope > .message-compact');
  let topRow = null;
  for (const row of rows) {
    const r = row.getBoundingClientRect();
    if (r.bottom > topEdge + 1) { topRow = row; break; }
  }
  if (!topRow) { floater.style.display = 'none'; return; }

  // Walk back to the run's originating full .message (which carries the avatar).
  let origin = topRow.classList.contains('message') ? topRow : null;
  if (!origin) {
    let p = topRow;
    while ((p = p.previousElementSibling)) {
      if (p.classList.contains('message')) { origin = p; break; }
      if (!p.classList.contains('message-compact')) break; // divider/marker → give up
    }
  }
  if (!origin) { floater.style.display = 'none'; return; }

  const av = origin.querySelector('.message-avatar-img') || origin.querySelector('.message-avatar');
  if (!av) { floater.style.display = 'none'; return; }
  const avRect = av.getBoundingClientRect();
  // Origin's own avatar still visible → no need to float a copy.
  if (avRect.bottom > topEdge + 2 && avRect.top >= cRect.top - 2) { floater.style.display = 'none'; return; }

  const oid = origin.dataset.msgId || origin.dataset.userId || '';
  if (floater._forId !== oid) {
    const clone = av.cloneNode(true);
    clone.style.display = '';
    clone.removeAttribute('loading');
    floater.replaceChildren(clone);
    floater._forId = oid;
  }

  const size = avRect.width || 40;
  let top = topEdge;
  // Slide out as the next author's run reaches the top.
  let next = origin;
  while ((next = next.nextElementSibling)) { if (next.classList && next.classList.contains('message')) break; }
  if (next && next.classList.contains('message')) {
    const nTop = next.getBoundingClientRect().top;
    if (nTop < topEdge + size) top = nTop - size;
  }
  floater.style.left = Math.round(avRect.left) + 'px';
...
// Shake the whole app briefly — the visual half of a DM nudge (#6), so desktop
// users (where navigator.vibrate is a no-op) still feel the buzz.
_shakeScreen() {
  const el = document.getElementById('app') || document.body;
  if (!el) return;
  el.classList.remove('ma-nudge-shake');
  void el.offsetWidth;            // restart the animation if it's already running
  el.classList.add('ma-nudge-shake');
  setTimeout(() => el.classList.remove('ma-nudge-shake'), 650);
},
...
/* ── Mobile App Bridge (Capacitor shell ↔ MSG Arena) ───── */
...
_setupMobileBridge() {
  ...
  // Notify the shell that MSG Arena is loaded and ready
