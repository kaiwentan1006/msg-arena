...
async switchChannel(code) {
  ...
  if (isDm && channel) {
    try {
      ...
      console.warn('[MSG Arena] DM partner key fetch failed, continuing unencrypted:', err);
    }
  }
...
_renderWebhookList(webhooks, channelCode) {
  ...
  // Guard both the container and the payload: a 'webhooks-list' event also
  // drives a second listener (the admin bots panel). If this one throws — no
  // #webhook-list in the DOM, or an undefined webhooks array — the Socket.IO
  // listener chain aborts and that second listener never runs. (#webhook-bug)
  if (!container) return;
  webhooks = webhooks || [];
  if (!webhooks.length) {
...
_renderChannels() {
  ...
  if (_hiddenExisting.length) {
    ...
    hiddenBar.innerHTML = `<svg class="hidden-bar-ico" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg><span>${t('channels.hidden_channels_count', { count: _hiddenExisting.length })}</span>`;
  ...
  if (dmList) {
    ...
    // Mirror the unread total onto the DM tab badge + the missed-messages banner.
    if (this._updateDmTabBadge) this._updateDmTabBadge(totalUnread);
    // Same for channels — a badge on the Channels tab shows unread in channels
    // you're not currently reading (e.g. while you're on the DMs tab).
    const totalChUnread = regularChannels.reduce((sum, ch) =>
      sum + ((ch.code in this.unreadCounts) ? this.unreadCounts[ch.code] : (ch.unreadCount || 0)), 0);
    if (this._updateChannelsTabBadge) this._updateChannelsTabBadge(totalChUnread);
    ...
    const renderDmItem = (ch) => {
      ...
      // DM row shows the partner's avatar + presence dot (not a bare "@").
      const tgt = ch.dm_target || {};
      const initial = (dmName.trim()[0] || '?').toUpperCase();
      const shape = tgt.avatar_shape ? ` avatar-${tgt.avatar_shape}` : '';
      const avatarHtml = tgt.avatar
        ? `<img class="dm-item-avatar${shape}" src="${this._escapeHtml(tgt.avatar)}" alt="">`
        : `<span class="dm-item-avatar dm-item-avatar-fallback${shape}" style="background-color:${this._getUserColor(dmName)}">${this._escapeHtml(initial)}</span>`;
      const statusHtml = `<span class="dm-item-status ${tgt.online ? 'online' : 'offline'}" title="${tgt.online ? 'Online' : 'Offline'}"></span>`;
      el.innerHTML = `
        <span class="dm-item-avatar-wrap">${avatarHtml}${statusHtml}</span>
...
_updateTabTitle() {
  ...
  // Include the server's display name so multiple MSG Arena tabs are easy to tell
...
_fireNativeNotification(message, channelCode, opts) {
  // Only pop a native OS notification when the app is NOT in the foreground —
  // i.e. minimised, closed to the tray, or a backgrounded browser tab (#3).
  // One authoritative gate here (rather than at each call site) — one caller
  // path was firing on every message even while the app was open. In the
  // desktop app the renderer reports document.hidden while the window is
  // minimised or hidden to the tray, so this covers desktop and web alike.
  if (!document.hidden) return;
