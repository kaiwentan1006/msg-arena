// ── Rank-reward cosmetics (mirrors src/rankPerks.js thresholds) ──────────
// Earned by leveling up: banners (solid → gradient → animated) + name effects.
// CSS for each id lives in msg-arena.css (.ma-banner-<id> / .ma-fx-<id>).
const RANK_BANNERS = [
  { id: 'default',       label: 'Default',      minLevel: 0 },
  { id: 'solid-slate',   label: 'Slate',        minLevel: 0 },
  { id: 'solid-cyan',    label: 'Cyan',         minLevel: 0 },
  { id: 'solid-crimson', label: 'Crimson',      minLevel: 0 },
  { id: 'solid-violet',  label: 'Violet',       minLevel: 0 },
  { id: 'grad-aurora',   label: 'Aurora',       minLevel: 5 },
  { id: 'grad-sunset',   label: 'Sunset',       minLevel: 5 },
  { id: 'grad-ocean',    label: 'Ocean',        minLevel: 5 },
  { id: 'grad-ember',    label: 'Ember',        minLevel: 5 },
  { id: 'anim-holo',     label: 'Holographic',  minLevel: 15 },
  { id: 'anim-pulse',    label: 'Pulse',        minLevel: 15 },
  { id: 'anim-flow',     label: 'Flow',         minLevel: 15 },
];
const RANK_NAME_FX = [
  { id: 'none',          label: 'None',          minLevel: 0 },
  { id: 'name-gradient', label: 'Gradient name', minLevel: 10 },
  { id: 'name-glow',     label: 'Glowing name',  minLevel: 20 },
];
const RANK_BANNER_IDS = new Set(RANK_BANNERS.map(b => b.id));
const RANK_FX_IDS = new Set(RANK_NAME_FX.map(n => n.id));
...
_renderPlayerCard(data) {
  ...
  const badgeHtml = badges.length
    ...
    : '';
  const lv = data.level;
  const streakChip = (lv && lv.streak > 0)
    ? `<span class="gs-streak" title="${lbl('users.gs_streak_longest', 'Longest streak')}: ${lv.longestStreak || lv.streak}">🔥 ${lv.streak}${lv.streak === 1 ? '' : ''} ${lbl('users.gs_streak_day', 'day streak')}</span>`
    : '';
  const levelHtml = (lv && lv.xp > 0)
    ...
    : '';
  // Country flag from a 2-letter code (regional-indicator codepoints; no lookup table).
  const _flag = (cc) => (cc && /^[A-Z]{2}$/.test(cc)) ? String.fromCodePoint(...[...cc].map(ch => 0x1F1E6 + ch.charCodeAt(0) - 65)) : '';
  const prof = data.profile || {};
  const locBits = [];
  if (prof.country) locBits.push(`${_flag(prof.country)} ${this._escapeHtml(prof.country)}`.trim());
  if (prof.languages && prof.languages.length) locBits.push(this._escapeHtml(prof.languages.map(x => String(x).toUpperCase()).join(' · ')));
  const locHtml = locBits.length ? `<div class="gs-loc">${locBits.join(' &nbsp;·&nbsp; ')}</div>` : '';
  const refHtml = (data.referrals > 0) ? `<div class="gs-loc gs-referrals">🎉 ${this._escapeHtml(String(data.referrals))} ${lbl('users.referrals', data.referrals === 1 ? 'friend recruited' : 'friends recruited')}</div>` : '';
  const gm = (data.games || []);
  const gamesHtml = gm.length
    ? `<div class="gs-games">${gm.slice(0, 10).map(g => `<span class="gs-game-chip" title="${this._escapeHtml(g.name)}${g.rank ? ' — ' + this._escapeHtml(g.rank) : ''}">${this._escapeHtml(g.icon || '🎮')} ${this._escapeHtml(g.name)}${g.rank ? `<b>${this._escapeHtml(g.rank)}</b>` : ''}</span>`).join('')}</div>`
    : '';
  ...
  const squadsHtml = sq.length
    ...
    : '';
  el.innerHTML = (chips.length || badges.length || gm.length || sq.length || locHtml || refHtml || (lv && lv.xp > 0))
...
_showProfilePopup(profile) {
  ...
  // Initial must track the DISPLAY name (what the avatar everywhere else shows),
  // not the immutable username — otherwise renaming ADMIN→"MSG ARENA" leaves the
  // avatar showing "M" while this modal still showed "A" (#14).
  ...
  // Action buttons
  const nickBtnLabel = currentNick ? t('users.edit_nickname') : t('users.set_nickname');
  const canMod = this.user.isAdmin || this._canModerate();
  const canPromote = this._hasPerm('promote_user');
  const gearVisible = !isSelf && (canMod || canPromote || this.user.isAdmin);
  const gearBtnHtml = gearVisible
    ? `<button class="profile-popup-action-btn profile-gear-btn" title="${this._escapeHtml(t('users.more_actions') || 'Moderation')}">${t('users.more_actions') || 'Moderation'}</button>`
    : '';
  const actionsHtml = isSelf
    ? `<button class="profile-popup-action-btn profile-edit-btn" id="profile-popup-edit-btn">${t('users.edit_profile')}</button><button class="profile-popup-action-btn profile-rewards-btn" id="profile-popup-rewards-btn">Rank Rewards</button><button class="profile-popup-action-btn profile-dm-btn" data-dm-uid="${profile.id}" title="Notes to self">Notes to self</button>`
  ...
  // Rank-reward cosmetics: a chosen banner preset paints the popup header, and a
  // name effect styles the display name. Both are validated ids from the server.
  const bannerId = RANK_BANNER_IDS.has(profile.profileBanner) && profile.profileBanner !== 'default' ? profile.profileBanner : '';
  const nameFxId = RANK_FX_IDS.has(profile.nameEffect) && profile.nameEffect !== 'none' ? profile.nameEffect : '';
  const bannerCls = bannerId ? ` ma-banner ma-banner-${bannerId}` : '';
  const bannerStyle = bannerId ? '' : `style="background:linear-gradient(135deg, ${color}44, ${color}22)"`;
  const nameFxCls = nameFxId ? ` ma-fx-${nameFxId}` : '';

  const popup = document.createElement('div');
  popup.id = 'profile-popup';
  popup.className = 'profile-popup';
  popup.innerHTML = `
    <div class="profile-popup-banner${bannerCls}" ${bannerStyle}>
    ...
    <div class="profile-popup-header">
      ...
      <div class="profile-popup-names">
        ${currentNick ? `<span class="profile-popup-nickname">${this._escapeHtml(currentNick)}</span>` : ''}
        <span class="profile-popup-displayname${nameFxCls}">${this._escapeHtml(profile.displayName)}</span>
  ...
  const rewardsBtnEl = popup.querySelector('#profile-popup-rewards-btn');
  if (rewardsBtnEl) {
    rewardsBtnEl.addEventListener('click', () => {
      this._closeProfilePopup();
      this._openRankRewardsModal(profile);
    });
  }
...
// ── Rank Rewards picker (cosmetics unlocked by level) ──────────────────
_openRankRewardsModal(profile) {
  const existing = document.getElementById('rank-rewards-modal');
  if (existing) existing.remove();
  const level = Number((profile && profile.level) || (this.user && this.user.level) || 0);
  let curBanner = (profile && profile.profileBanner) || (this.user && this.user.profileBanner) || 'default';
  let curFx = (profile && profile.nameEffect) || (this.user && this.user.nameEffect) || 'none';
  const esc = (s) => this._escapeHtml(String(s));

  const swatch = (b) => {
    const locked = level < b.minLevel;
    const sel = (b.id === curBanner || (b.id === 'default' && !RANK_BANNER_IDS.has(curBanner)));
    const cls = b.id === 'default' ? '' : ` ma-banner ma-banner-${b.id}`;
    return `<button class="rr-swatch${sel ? ' selected' : ''}${locked ? ' locked' : ''}" data-banner="${b.id}" ${locked ? 'disabled' : ''} title="${esc(b.label)}${locked ? ` — unlocks at Level ${b.minLevel}` : ''}">
      <span class="rr-swatch-fill${cls}"></span>
      <span class="rr-swatch-label">${esc(b.label)}</span>
      ${locked ? `<span class="rr-lock">Lv ${b.minLevel}</span>` : ''}
    </button>`;
  };
  const fxRow = (n) => {
    const locked = level < n.minLevel;
    const sel = (n.id === curFx || (n.id === 'none' && !RANK_FX_IDS.has(curFx)));
    const demoCls = n.id === 'none' ? '' : ` ma-fx-${n.id}`;
    return `<button class="rr-fx${sel ? ' selected' : ''}${locked ? ' locked' : ''}" data-fx="${n.id}" ${locked ? 'disabled' : ''}>
      <span class="rr-fx-demo${demoCls}">${esc(this.user.displayName || this.user.username || 'Name')}</span>
      <span class="rr-fx-label">${esc(n.label)}${locked ? ` · Lv ${n.minLevel}` : ''}</span>
    </button>`;
  };

  const modal = document.createElement('div');
  modal.id = 'rank-rewards-modal';
  modal.className = 'modal-overlay';
  modal.style.display = 'flex';
  modal.innerHTML = `
    <div class="modal rank-rewards-box">
      <h3>Rank Rewards</h3>
      <p class="settings-hint" style="margin:0 0 12px">You're <strong>Level ${level}</strong>. Keep chatting, playing and recruiting to climb — new banners and name effects unlock as you rank up.</p>
      <div class="rr-section-label">Profile banner</div>
      <div class="rr-swatch-grid">${RANK_BANNERS.map(swatch).join('')}</div>
      <div class="rr-section-label" style="margin-top:14px">Name effect</div>
      <div class="rr-fx-grid">${RANK_NAME_FX.map(fxRow).join('')}</div>
      <div class="modal-actions" style="margin-top:16px">
        <button class="btn-sm btn-accent" id="rank-rewards-close">Done</button>
      </div>
    </div>`;
  document.body.appendChild(modal);
  ...
  document.getElementById('rank-rewards-close').addEventListener('click', close);

  modal.querySelectorAll('.rr-swatch:not(.locked)').forEach(btn => {
    btn.addEventListener('click', () => {
      const id = btn.dataset.banner;
      curBanner = id;
      modal.querySelectorAll('.rr-swatch').forEach(b => b.classList.toggle('selected', b === btn));
      if (this.user) this.user.profileBanner = id === 'default' ? null : id;
      this.socket.emit('set-profile-banner', { banner: id });
    });
  });
  modal.querySelectorAll('.rr-fx:not(.locked)').forEach(btn => {
    btn.addEventListener('click', () => {
      const id = btn.dataset.fx;
      curFx = id;
      modal.querySelectorAll('.rr-fx').forEach(b => b.classList.toggle('selected', b === btn));
      if (this.user) this.user.nameEffect = id === 'none' ? null : id;
      this.socket.emit('set-name-effect', { effect: id });
    });
  });
},
