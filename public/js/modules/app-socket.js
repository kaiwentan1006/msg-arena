...
_setupSocketListeners() {
  ...
  // in the background. It only ever hit web users because MSG Arena Desktop
  ...
  // unlocks the PC and clicks back into the MSG Arena window, focus fires even
  ...
  this.socket.on('channels-list', (channels) => {
    ...
    if (inviteCode && !this._inviteHandled) {
      ...
      // Invite links are how people join servers in the self-hosted model, so
      // opening one also REMEMBERS this server in your rail — no hand-typing a
      // URL. Fully decentralized: the link's own origin IS the server, nothing
      // central is contacted. (see _addServerFromInvite)
      this._addServerFromInvite?.();
  ...
  this.socket.on('player-card', (data) => {
    if (this._renderPlayerCard) this._renderPlayerCard(data);
  });

  this.socket.on('roles:self-list', (data) => { if (this._renderSelfRoles) this._renderSelfRoles(data); });

  this.socket.on('achievement-earned', (data) => {
    for (const a of (data && data.achievements) || []) {
      const msg = (t('achievements.unlocked', { name: a.name }) || `Achievement unlocked: ${a.name}`);
      this._showToast(`${a.icon} ${msg}`, 'success');
    }
  });

  // XP / leveling — celebrate a level-up; stash live XP for any self display.
  this.socket.on('xp-levelup', (data) => {
    const lvl = data && data.level;
    const msg = t('xp.levelup', { level: lvl });
    this._showToast(`⭐ ${(msg && msg !== 'xp.levelup') ? msg : `Level up! You reached level ${lvl}`}`, 'success');
    try { this.notifications && this.notifications.playDirect && this.notifications.playDirect('mention'); } catch {}
    // Level role reward(s) unlocked — call them out; they land on your profile.
    const roles = (data && data.rewardedRoles) || [];
    if (roles.length) {
      const names = roles.map(r => this._escapeHtml ? this._escapeHtml(r.name) : r.name).join(', ');
      this._showToast(`🎖️ ${t('xp.role_unlocked', { role: names }, `Unlocked role: ${names}`)}`, 'success');
      // Refresh self-view so the new role shows immediately if the profile is open.
      try { this.socket.emit('roles:self-list'); } catch {}
    }
  });
  // Referral rank — you were rewarded for friends who joined via your invites.
  this.socket.on('xp-referral', (data) => {
    if (!data || !(data.newReferrals > 0)) return;
    const n = data.newReferrals, total = data.totalReferrals;
    const msg = t('xp.referral', { n, total }, `🎉 +${n} referral${n === 1 ? '' : 's'}! You've recruited ${total} — rank up!`);
    this._showToast(msg, 'success');
    try { this.notifications && this.notifications.playDirect && this.notifications.playDirect('mention'); } catch {}
  });
  // Daily streak — first activity of a new day earned a bonus.
  this.socket.on('xp-streak', (data) => {
    if (!data || !(data.streak > 1)) return;
    const msg = t('xp.streak', { days: data.streak, bonus: data.bonus }, `🔥 ${data.streak}-day streak! +${data.bonus} XP`);
    this._showToast(msg, 'success');
  });
  this.socket.on('xp-update', (data) => { if (data) this._myXp = data; });

  // A global game record was broken — announce it to everyone to spark
  // competition (#8). Shown as a toast; the breaker gets a personal variant.
  this.socket.on('record-broken', (d) => {
    if (!d || !d.gameName) return;
    let score = d.score;
    try { score = Number(d.score).toLocaleString(); } catch {}
    const isMe = d.userId === this.user?.id;
    const msg = isMe
      ? `New record! You broke the ${d.gameName} record with ${score}.`
      : `${d.username || 'Someone'} just broke the ${d.gameName} record — ${score}!`;
    try { this._showToast(msg, 'success'); } catch {}
  });

  // DM Nudge received (#6): buzz the device, shake the screen, and toast — plus a
  // native notification if the app is backgrounded (that call self-gates on hidden).
  this.socket.on('dm:nudged', (d) => {
    if (!d) return;
    try { navigator.vibrate && navigator.vibrate([120, 60, 120]); } catch {}
    try { this._shakeScreen?.(); } catch {}
    try { this.notifications?.play?.('mention'); } catch {}
    try { this._showToast(`${d.from || 'Someone'} nudged you`, 'info'); } catch {}
    try { this._fireNativeNotification?.({ user_id: d.fromId, content: 'nudged you' }, d.code, { isDm: true }); } catch {}
  });
  this.socket.on('dm:nudge-sent', () => { try { this._showToast('Nudge sent', 'success'); } catch {} });

  // DM read receipts (#5): the recipient read up to lastReadMessageId — upgrade
  // our sent-ticks to read-ticks in that DM.
  this.socket.on('dm:read', (d) => {
    if (!d || typeof d.code !== 'string') return;
    if (!this._dmReadUpTo) this._dmReadUpTo = {};
    const prev = this._dmReadUpTo[d.code] || 0;
    this._dmReadUpTo[d.code] = Math.max(prev, d.lastReadMessageId || 0);
    try { this._applyDmReceipts?.(d.code); } catch {}
  });

  this.socket.on('user-profile', (profile) => {
    // A hover fetch and a click fetch for the same user can race (you hover, then
    // click before the hover's 350ms request returns), so two 'user-profile'
    // responses arrive and the popup closes+reopens — a visible double flicker
    // (#13). Coalesce a duplicate for the same user within a short window when a
    // permanent popup is already up. Hover popups (translucent) are never skipped.
    if (profile && !this._isHoverPopup) {
      const now = Date.now();
      if (this._lastProfileShownId === profile.id &&
          (now - (this._lastProfileShownAt || 0)) < 600 &&
          document.getElementById('profile-popup')) {
        this._lastProfileShownAt = now;
        return;
      }
      this._lastProfileShownId = profile.id;
      this._lastProfileShownAt = now;
    }
  ...
  // Bulk purge: the server emits this to every admin, but nothing listened, so
  // other admins' open member lists stayed stale until a manual refresh. Mirror
  // the single-delete handling across all removed ids.
  this.socket.on('users-bulk-removed', (data) => {
    const ids = Array.isArray(data && data.ids) ? data.ids : [];
    if (ids.length && this._allMembersData) {
      const removed = new Set(ids);
      this._allMembersData = this._allMembersData.filter(m => !removed.has(m.id));
      this._filterAllMembers();
    }
  });
  ...
  // ── Self-declared game IDs (own list, for the settings editor) ──
  this.socket.on('gameid:list', (data) => {
    if (!data || data.userId !== this.user?.id) return;   // only my own list drives the editor
    this._myGameIds = data.ids || [];
    // Refresh just the editor if the Connections settings are open.
    const host = document.getElementById('connections-list');
    if (host && document.querySelector('.gameid-editor')) {
      document.querySelectorAll('.gameid-editor').forEach(el => el.remove());
  ...
  // MSG Arena window itself became Steam's sign-in page, and once the flow
