...
(function() {
  ...
  try {
    ...
    if (!_z) {
      ...
      _z = _legacyZoom[localStorage.getItem('haven-fontsize')] || 0;
    }
    if (_z) {
      // Explicit user zoom always wins.
      _z = Math.min(150, Math.max(70, _z));
      document.documentElement.style.setProperty('--ui-scale', _z + '%');
    } else {
      // No manual zoom → scale the whole UI proportionally to the viewport so
      // elements stay in proportion to the device instead of being oversized on
      // phones/tablets (#8). Scale to whichever axis is more constrained: width
      // catches narrow/portrait phones and tablets, height catches short
      // landscape phones (e.g. 1024x460) where a width-only rule left the UI
      // magnified and controls cut off. Base 1440x720 = 100%; a generous height
      // reference keeps ordinary 768p laptops at ~full size, floored at 60% so
      // small screens shrink hard enough to fit without becoming illegible.
      var _applyUiScale = function () {
        try { if (parseInt(localStorage.getItem('haven-zoom'), 10)) return; } catch (e) {}
        ...
        var pct = Math.min(vw / 1440, vh / 720) * 100;
        pct = Math.round(Math.max(60, Math.min(100, pct)));
        document.documentElement.style.setProperty('--ui-scale', pct + '%');
      };
      _applyUiScale();
      window.addEventListener('resize', _applyUiScale);
      if (window.visualViewport) window.visualViewport.addEventListener('resize', _applyUiScale);
    }
  } catch (e) {}
