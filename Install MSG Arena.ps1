# ═══════════════════════════════════════════════════════════
# MSG Arena — Graphical Installer (PowerShell WPF)
# Usage: powershell -ExecutionPolicy Bypass -File "Install MSG Arena.ps1"
# ═══════════════════════════════════════════════════════════
param([switch]$Silent)
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="MSG Arena Installer" Width="640" Height="520"
  ...
  <Grid>
    ...
    <Grid Margin="48,32,48,24">
      ...
      <StackPanel x:Name="page0" Visibility="Visible">
        ...
        <TextBlock Text="Welcome to MSG Arena" FontSize="28" FontWeight="Bold"
          ...
          <StackPanel>
            ...
            <StackPanel Orientation="Horizontal" Margin="0,8,0,0">
              ...
              <StackPanel Margin="10,0,0,0">
                <TextBlock x:Name="lblPreview" Text="MSG Arena" FontSize="16" FontWeight="SemiBold"/>
      ...
      <StackPanel x:Name="page4" Visibility="Collapsed">
        <TextBlock Text="Installing MSG Arena…" FontSize="22" FontWeight="Bold" Margin="0,20,0,0"/>
      ...
      <StackPanel x:Name="page5" Visibility="Collapsed">
        ...
        <TextBlock Text="MSG Arena is installed!" FontSize="28" FontWeight="Bold"
        ...
        <StackPanel Orientation="Horizontal" HorizontalAlignment="Center" Margin="0,28,0,0">
          <Button x:Name="btnLaunch" Content="🚀  Launch MSG Arena" Style="{StaticResource GreenBtn}"
                  Margin="0,0,10,0"/>
...
$ui['btnNext1'].Add_Click({
    ...
    if ([string]::IsNullOrWhiteSpace($name)) { $name = 'MSG Arena' }
    Show-Page 2
})
...
$ui['txtServerName'].Add_TextChanged({
    ...
    if ([string]::IsNullOrWhiteSpace($t)) { $t = 'MSG Arena' }
...
$ui['btnNext3'].Add_Click({
    ...
    if ([string]::IsNullOrWhiteSpace($serverName)) { $serverName = 'MSG Arena' }
    ...
    if (!(Test-Path "$certDir\cert.pem")) {
        ...
        if ($opensslPath) {
            $sslOutput = & openssl req -x509 -newkey rsa:2048 -keyout "$certDir\key.pem" -out "$certDir\cert.pem" -days 3650 -nodes -subj "/CN=MSG Arena" 2>&1
            if (Test-Path "$certDir\cert.pem") {
...
"@
    ...
    # Mark tunnel configured so Start MSG Arena.bat skips the prompt
    ...
    try {
        ...
        $lnk.TargetPath = "$HAVEN_DIR\Start MSG Arena.bat"
        ...
        $lnk2.TargetPath = "$HAVEN_DIR\Start MSG Arena.bat"
    ...
    if ($tunnelEnabled -eq 'true') {
        $doneText += "`nA $tunnelProvider tunnel will start automatically when you launch MSG Arena."
    }
    $ui['lblDone'].Text = $doneText
    $ui['lblShareLink'].Text = "Double-click 'Start MSG Arena' to begin!"
    Show-Page 5
})

# ── Final page actions ─────────────────────────────────────
$ui['btnLaunch'].Add_Click({
    Start-Process "$HAVEN_DIR\Start MSG Arena.bat"
    $window.Close()
})
