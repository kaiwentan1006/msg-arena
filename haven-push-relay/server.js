...
if (!PUSH_KEY) {
  ...
  console.log(`MSG Arena .env: FCM_RELAY_URL=http://YOUR_HOST:${PORT}/sendPush`);
  console.log(`MSG Arena .env: FCM_PUSH_KEY=${generated}\n`);
  process.env.HAVEN_PUSH_KEY = generated;
}
// Constant-time secret comparison so the push key can't be recovered by timing
// the 403 response character-by-character.
...
app.post('/sendPush', async (req, res) => {
  ...
  if (!provided || !safeEqual(String(provided), String(key))) return res.status(403).json({ error: 'Invalid push key' });
...
app.listen(PORT, () => {
  console.log(`MSG Arena Push Relay running on port ${PORT}`);
