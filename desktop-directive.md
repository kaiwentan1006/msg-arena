# MSG Arena Desktop App — Directive

> **Purpose:** This document captures the full design intent, architecture, and requirements
> for the MSG Arena Desktop App. The desktop app is being rebuilt **from scratch** in a **separate
> repository**, completely independent from the MSG Arena server repo.

---

## 1. Vision

A native desktop client for MSG Arena that makes the entire experience brainlessly simple:
install, launch, and you're chatting. No terminal commands, no config files, no friction.

The desktop app is **not** part of the MSG Arena server. It connects **to** a MSG Arena server the same
...
1. **Admin launches their MSG Arena server** the normal way (Start MSG Arena.bat, `npm start`, Docker, etc.)
2. **Browser users** see a popup prompting them to install the MSG Arena Desktop App.
...
3. On success, loads the MSG Arena web UI inside the native window.
4. Subsequent launches skip the Welcome Screen and go straight to the server.

### "Host My Own" Path

1. App **auto-detects** whether a MSG Arena server is installed on this machine.
...
3. **If server NOT found:** Informs the user. Offers two options:
   ...
   - **"Start Server Setup"** — Begins a one-click server installation wizard:
     ...
     - Clones or downloads the MSG Arena server
     - Runs `npm install`
...
### Window & Chrome
- Native window with app icon, proper title ("MSG Arena"), dark theme
...
- Accept self-signed certificates for localhost only (MSG Arena generates self-signed certs)
...
- Start/stop MSG Arena server as a managed child process
- Auto-detect MSG Arena server installation on the local machine
...
- Mic and speaker dropdown menus injected into MSG Arena's Settings modal
...
│   └── renderer/                   # Scripts injected into MSG Arena's web UI
...
- `appId`: `com.msgarena.desktop`
- `productName`: `MSG Arena`
...
## 6. Integration Points with MSG Arena Server

The desktop app is a **client** of the MSG Arena server. It does NOT modify the server.
...
- Standard MSG Arena web UI (loaded in the Electron BrowserWindow)
...
1. **Completely separate repo** — The desktop app has its own git repository, its own
   package.json, its own CI/CD. It connects to MSG Arena like any browser client.
