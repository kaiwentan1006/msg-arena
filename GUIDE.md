...
```bash
docker pull ghcr.io/your-org/msg-arena:latest
docker run -d -p 3000:3000 -v haven_data:/data ghcr.io/your-org/msg-arena:latest
```

**Option B — Build from source**:
```bash
git clone https://github.com/your-org/MSG-Arena.git
cd MSG-Arena
docker compose up -d
```
...
```yaml
volumes:
  - /path/to/your/msg-arena-data:/data
```
...
```powershell
New-NetFirewallRule -DisplayName "MSG_Arena_Chat" -Direction Inbound -LocalPort 3000 -Protocol TCP -Action Allow
```
...
> `.env`. Example:
> ```
> Environment=HAVEN_DATA_DIR=/opt/msg-arena-data
> ```
...
> ```
> PUBLIC_URL=https://msg.example.com:8443
> ```
...
3. Set a unique `HAVEN_DATA_DIR` in each `.env` (e.g. `HAVEN_DATA_DIR=C:\MSGArenaData\server1`).
...
> **Looking for ready-made bots and webhooks?** The community library at [**your-org/MSG-Arena-community**](https://github.com/your-org/MSG-Arena-community) collects user-contributed integrations you can deploy as-is — a GitHub releases poster, etc. PRs welcome there if you've built one of your own. See its [`CONTRIBUTING.md`](https://github.com/your-org/MSG-Arena-community/blob/main/CONTRIBUTING.md) for how to submit.
