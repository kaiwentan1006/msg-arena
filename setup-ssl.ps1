<# 
  MSG Arena — Let's Encrypt SSL Certificate Setup (Windows)

  This script uses win-acme (ACME client for Windows) to obtain a free
  Let's Encrypt certificate for your MSG Arena server.
  ...
  The certificate files (cert.pem + key.pem) will be placed in MSG Arena's
...
Write-Host "  MSG Arena SSL Certificate Setup" -ForegroundColor Cyan
Write-Host "=====================================" -ForegroundColor Cyan
Write-Host ""
...
switch ($method) {
  "1" {
    ...
    # win-acme outputs files with different names — rename to what MSG Arena expects
  ...
  "2" {
    ...
    # Copy certs to MSG Arena's directory
...
if ($certExists -and $keyExists) {
  ...
  Write-Host ""
  Write-Host "  MSG Arena will auto-detect these on next startup." -ForegroundColor Cyan
  Write-Host "  Just run: Start MSG Arena.bat" -ForegroundColor Cyan
  Write-Host ""
  if ($method -eq "1") {
