...
- **Docker update instructions** — `docker-compose.yml` now defaults to the pre-built image (`ghcr.io/kaiwentan1006/msg-arena:latest`), fixing the issue where `docker compose up -d` would rebuild from source rather than use the pulled image. Update instructions updated throughout.
...
### Added
- **MSG Arena Android beta sign-up** — a new green "Android Beta" pill button in the top bar opens a sign-up popup directing users to [your-domain.example](https://msg-arena.up.railway.app) to request access to the MSG Arena Android closed beta on Google Play. The popup appears automatically for first-time visitors (after the desktop promo, if applicable) and can be permanently dismissed via "Don't show this again".
- **Android beta on the website** — the [MSG Arena website](https://msg-arena.up.railway.app/) now features a dedicated Android banner section with sign-up link, plus a download card in the download section.
...
- **MSG Arena Desktop beta** — standalone Electron desktop app now available at [github.com/kaiwentan1006/MSG-Arena-Desktop](https://github.com/kaiwentan1006/MSG-Arena-Desktop). Per-app audio, native notifications, system tray, one-click install.
...
- **Docker image publishing** — pre-built Docker images are now automatically pushed to GitHub Container Registry on every release (`ghcr.io/kaiwentan1006/msg-arena:latest`). No build step needed.
...
### Fixed
- **SSL_ERROR_RX_RECORD_TOO_LONG on Windows** — `Start MSG Arena.bat` always opened the browser with `https://` even when the server was running in HTTP mode (no valid SSL certs). The batch file now detects the actual protocol and opens the correct URL. ([#2](https://github.com/kaiwentan1006/MSG-Arena/issues/2))
