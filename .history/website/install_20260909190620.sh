#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════
# MSG Arena — One-Click Installer Bootstrap (Linux / macOS)
# Run: curl -fsSL https://kaiwentan1006.github.io/MSG-Arena/install.sh | bash
#  or: wget -qO- https://kaiwentan1006.github.io/MSG-Arena/install.sh | bash
# ═══════════════════════════════════════════════════════════
set -e
...
INSTALL_DIR="$HOME/MSG-Arena"

echo ""
echo -e "${GREEN}${BOLD}  ========================================${NC}"
echo -e "${GREEN}${BOLD}    MSG ARENA — One-Click Installer${NC}"
echo -e "${GREEN}${BOLD}  ========================================${NC}"
echo ""
echo "  This will set up MSG Arena, your private"
...
if [ "$(uname -s)" = "Linux" ]; then
    ...
    if [ "$_ARCH" = "x86_64" ] || [ "$_ARCH" = "aarch64" ]; then
        ...
        echo "    https://github.com/kaiwentan1006/MSG-Arena/releases/latest"
...
if ! command -v node &> /dev/null; then
    echo ""
    echo -e "  ${CYAN}MSG Arena needs Node.js to run.${NC}"
    ...
    if command -v apt-get &> /dev/null; then
        ...
        if [ -s "$NVM_DIR/nvm.sh" ]; then
            ...
            echo "    2. Download the MSG Arena AppImage (no Node.js needed):"
            echo "         https://github.com/kaiwentan1006/MSG-Arena/releases/latest"
            exit 1
        fi
    fi
...
# ── Step 2: Download MSG Arena ───────────────────────────────
echo "  [2/3] Downloading MSG Arena..."

if [ -f "$INSTALL_DIR/package.json" ]; then
    echo "        MSG Arena already downloaded, updating..."
    ...
    if command -v git &> /dev/null; then
        ...
        git clone --depth 1 https://github.com/kaiwentan1006/MSG-Arena.git "$INSTALL_DIR"
    else
        echo "        Downloading ZIP from GitHub..."
        TMP_ZIP=$(mktemp /tmp/haven-XXXXXX.zip)
        curl -fsSL "https://github.com/kaiwentan1006/MSG-Arena/archive/refs/heads/main.zip" -o "$TMP_ZIP"
        unzip -q "$TMP_ZIP" -d /tmp
        mv /tmp/MSG-Arena-main "$INSTALL_DIR"
        rm -f "$TMP_ZIP"
    fi
fi
...
echo "        MSG Arena ready at $INSTALL_DIR"
echo ""
