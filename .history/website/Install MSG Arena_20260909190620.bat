@echo off
:: ═══════════════════════════════════════════════════════════
:: MSG Arena — One-Click Installer Bootstrap
:: Download this file and double-click to install MSG Arena.
:: Everything is handled automatically.
:: ═══════════════════════════════════════════════════════════
title MSG Arena Installer
color 0A
echo.
echo  ========================================
echo       MSG ARENA — One-Click Installer
echo  ========================================
echo.
...
set "INSTALL_DIR=%USERPROFILE%\MSG-Arena"
echo  MSG Arena will be installed to:
echo    %INSTALL_DIR%
echo.
...
echo.
echo  MSG Arena needs Node.js to run. Don't worry,
...
if %ERRORLEVEL% EQU 0 (
    ...
    git clone --depth 1 https://github.com/kaiwentan1006/MSG-Arena.git "%INSTALL_DIR%" 2>nul
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$ProgressPreference = 'SilentlyContinue';" ^
  "Invoke-WebRequest -Uri 'https://github.com/kaiwentan1006/MSG-Arena/archive/refs/heads/main.zip' -OutFile $zip -UseBasicParsing;" ^
  "Expand-Archive -Path $zip -DestinationPath $env:TEMP -Force;" ^
  "if (Test-Path '%INSTALL_DIR%') { Remove-Item '%INSTALL_DIR%' -Recurse -Force };" ^
  "Move-Item \"$env:TEMP\MSG-Arena-main\" '%INSTALL_DIR%';" ^
  "Remove-Item $zip -Force -ErrorAction SilentlyContinue;" ^
  "Write-Host '  [OK] MSG Arena downloaded!';"
