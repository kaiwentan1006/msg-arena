...
**Optional feature variables (set only if you want the feature):**

| Variable | Value | Enables |
|---|---|---|
| `UPDATE_REPO` | `owner/repo` (e.g. `yourname/MSG-Arena-Desktop`) | The **/download** page's working per-OS buttons + the in-app "update available" check. Point it at the GitHub repo where you publish desktop/mobile releases. |
| `GOOGLE_SITE_VERIFICATION` | the token Google Search Console gives you | Injects the `google-site-verification` `<meta>` so you can verify ownership. See `docs/google-search-console.md`. |
| `NOINDEX` | `true` | Keep the **whole** site out of search engines (private community). |
| `DIRECTORY_LISTING` | `true` | Make this server the public **server-directory hub**. See `docs/server-directory.md`. |
| `REGISTRY_URL` | `https://your-hub` | List this server in another hub's directory. |
| `TURN_URL` + `TURN_SECRET` | your coturn server | Reliable cross-NAT voice (see the Voice section below). |
...
> only triggers on Railway's own variables. On a VPS you don't need a "volume" —
> just don't set `HAVEN_DATA_DIR` and the DB lives in `~/.haven`, or set it to any
> path on a persistent disk.
