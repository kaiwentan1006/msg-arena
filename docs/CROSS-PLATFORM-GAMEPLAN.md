# MSG Arena Cross-Platform Compatibility Gameplan

## Problem Statement

Most MSG Arena users run the **Docker build** and many use **Linux**. Three key features currently have platform-specific limitations:
...
```
[Client A: Browser/Desktop]  ←→  [MSG Arena Server (Docker)]  ←→  [Client B: Browser/Desktop]
...
**Current state:** MSG Arena already supports Web Push notifications via VAPID keys + service worker (`sw.js`). This works for:
...
1. **Graceful degradation in web/Docker clients** (Priority: LOW)
   ...
   - Show a message: "Per-app audio sharing requires the MSG Arena Desktop app"
...
### The Problem
Screen sharing requires WebRTC, which MSG Arena already uses for voice chat. The **signaling** goes through the Socket.IO server (works from Docker). The **media** goes peer-to-peer or through a TURN server.
...
2. **TURN server for Docker deployments** (Priority: HIGH)
   ...
   - MSG Arena already supports TURN configuration (`TURN_URL`, `TURN_USERNAME`, `TURN_CREDENTIAL` env vars)
...
3. **Add coturn to docker-compose.yml** (Priority: HIGH)
   ...
   realm=msg.example.com
   server-name=msg.example.com
   use-auth-secret
...
```env
# MSG Arena Server Configuration
PORT=3000
...
TURN_URL=turn:msg.example.com:3478
TURN_USERNAME=
TURN_CREDENTIAL=
...
Most Docker users will run MSG Arena behind nginx/Traefik/Caddy. Document:
...
1. Optional coturn auto-configuration via MSG Arena admin panel
