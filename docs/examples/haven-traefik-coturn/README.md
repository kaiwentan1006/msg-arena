# MSG Arena Docker Deployment
- This setup runs MSG Arena, Traefik (reverse proxy), and coturn (TURN relay) together in a single Docker Compose stack

## Requirements
- A Linux server with Docker installed (a VPS or any internet-facing machine where you can open ports)
- A domain name pointing to your server's public IP (e.g. `msg.example.com`)
...
```
git clone https://github.com/your-org/MSG-Arena
cd MSG-Arena/docs/examples/haven-traefik-coturn
```
...
docker compose up -d
```
- Once all images have pulled and started, your MSG Arena server should be reachable
...
- Once MSG Arena loads, create your admin account and log in
...
## How It Works
- Traefik sits in front of MSG Arena and handles all incoming HTTP and HTTPS connections
...
- Users connecting to your MSG Arena URL will get routed to your MSG Arena container
