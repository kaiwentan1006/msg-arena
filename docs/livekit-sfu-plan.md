...
- No native GPU encoding pipeline (the issue thread brings this up — that's an unrelated MSG Arena-Desktop project, not this one).
...
| Fit for MSG Arena's audience | Lower-friction self-host | Slightly better Node integration |

LiveKit wins on operational simplicity, which is the deciding factor for MSG Arena's "self-host on a $5 VPS" audience. Per an upstream maintainer's comment in #76: "LiveKit ships as one binary — operationally simpler for self-hosters."
...
1. Mints a LiveKit access token (room = `${LIVEKIT_ROOM_PREFIX}-${channelCode}`, identity = userId, permissions derived from MSG Arena role).
...
services:
  haven:
    # ... existing ...
    environment:
      # Auto-injected by MSG Arena on first SFU opt-in (do not edit by hand):
      - LIVEKIT_API_KEY=
      - LIVEKIT_API_SECRET=
      - LIVEKIT_WS_URL=
...
**Option B (even more invisible, more work): MSG Arena shells out to `docker compose up livekit`.**

Requires the MSG Arena container to have docker socket access. Risky. Recommend Option A.
...
4. **Health check.** After the admin clicks "I've done that", MSG Arena probes `LIVEKIT_WS_URL` and shows a green dot if reachable, red dot + diagnostic hint if not. The toggle stays off until the probe is green at least once. (Prevents enabling SFU before the container is up, which would just break voice for everyone.)
...
| `package.json` (web side, none — `livekit-client` loaded from CDN OR vendored under `public/vendor/livekit-client.min.js` to avoid CDN runtime dep) | vendor the build to keep MSG Arena offline-installable |
...
Most MSG Arena self-hosters are already running behind a reverse proxy (Caddy / Traefik / nginx) for HTTPS on the main 3000 port. We document two patterns in `docs/examples/`:
...
1. **Mobile.** Is the MSG Arena mobile client able to load `livekit-client`? If it's a thin WebView wrapper, yes. If it's React Native, the LiveKit RN SDK works but is a bigger lift. **Action: confirm before Phase 1.**
...
3. **TURN coexistence.** MSG Arena already supports `TURN_URL` for the mesh path. LiveKit can share that TURN server (it accepts external TURN config). Plumb the existing `TURN_URL` env into the LiveKit config so admins don't configure it twice.
...
- Switching MSG Arena to SFU-by-default. Don't.
- Building our own SFU. Don't.
- Tying MSG Arena to LiveKit Cloud / any hosted service. The provider must work fully self-hosted.
