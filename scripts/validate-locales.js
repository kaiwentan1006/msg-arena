...
// Scan the HTML for data-i18n* attributes and report any key that en.json is
// missing. Those keys can never be translated in ANY locale (the runtime falls
// back to English), so it's an en.json-completeness gap, not a per-locale one.
// Reported as a warning to match this tool's "warnings don't fail" philosophy.
function checkHtmlKeys(masterShape) {
  const PUBLIC_DIR = path.join(__dirname, '..', 'public');
  const htmlFiles = ['app.html', 'index.html'].map(f => path.join(PUBLIC_DIR, f));
  const attrRe = /data-i18n(?:-html|-title|-placeholder|-aria-label)?="([^"]+)"/g;
  ...
  for (const file of htmlFiles) {
    ...
    while ((m = attrRe.exec(html))) {
      const key = m[1];
      if (!masterShape.has(key)) missing.add(key);
    }
  }
  return missing;
}
...
function main() {
  ...
  const htmlMissing = checkHtmlKeys(master);
  if (htmlMissing.size > 0) {
    console.log(`⚠ en.json is missing ${htmlMissing.size} key(s) referenced by data-i18n in HTML — those UI strings can't be translated in any locale (English is shown). Add them to en.json.`);
  }
