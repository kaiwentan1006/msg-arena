...
# Data (database, .env, certs, uploads) is stored in /data so it survives
# container rebuilds. Provide /data as a volume: `docker compose` uses the
# haven_data named volume; on Railway, attach a Volume to the service at /data.
#
...
USER root
EXPOSE 3000 3001
# NOTE: no `VOLUME ["/data"]` here. Docker Compose mounts the named volume
# haven_data at /data (see docker-compose.yml), and Railway rejects the VOLUME
# instruction — on Railway attach a Volume to the service at /data instead.
