@echo off
title MSG Arena Server
color 0A
echo.
echo  ========================================
echo       MSG ARENA - Private Chat Server
echo  ========================================
echo.
...
:: Kill any existing MSG Arena server on the configured port
...
echo.
echo  [OK] Node.js installed! Close this window and double-click Start MSG Arena again.
...
if defined NODE_MAJOR (
    if %NODE_MAJOR% GEQ 27 (
        color 0E
        echo.
        echo  [!] WARNING: Node.js v%NODE_MAJOR% detected. MSG Arena is tested on Node 18-26.
...
if not defined OPENSSL_CMD (
    ...
    echo      MSG Arena will run in HTTP mode. See README for details.
...
"%OPENSSL_CMD%" req -x509 -newkey rsa:2048 -keyout "%HAVEN_DATA%\certs\key.pem" -out "%HAVEN_DATA%\certs\cert.pem" -days 3650 -nodes -subj "/CN=MSG Arena" -addext "subjectAltName=DNS:localhost,IP:127.0.0.1,IP:%LOCAL_IP%"
...
if "%HAVEN_PROTO%"=="https" (
    echo  ========================================
    echo    MSG Arena is LIVE on port %HAVEN_PORT% ^(HTTPS^)
    echo  ========================================
    echo.
    ...
    echo  ========================================
    echo    MSG Arena is LIVE on port %HAVEN_PORT% ^(HTTP^)
    echo  ========================================
    echo.
